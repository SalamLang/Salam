...
Salam is flexible about one surface detail: a call's arguments may be wrapped
in parentheses (\code{println("hi")}) or not (\code{println "hi"}). Blocks
themselves, though, always open with a colon and close with \code{end}
there is no curly-brace alternative. The two call styles are equivalent; the
bare-arguments form is the style used by most of the examples that ship with
Salam, so it is the style we adopt here. Pick one and be consistent and
remember the \code{salam fmt} command, which reformats your code into the
canonical style automatically.
\end{deep}
