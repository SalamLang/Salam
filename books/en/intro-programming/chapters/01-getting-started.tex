...
program into \textsc{C} source code and then invokes a C compiler (\code{tcc} by
default, but \code{gcc} or \code{clang} work too) to produce a native
executable. There is also a built-in \emph{interpreter} that can run your
...
There is a long-standing tradition among programmers: the very first program you
...
The simplest way to see the result is to use Salam's built-in interpreter, which
...
There is also a one-shot \code{run} command that builds and immediately runs
...
what you use to deliver a finished program. \code{run} is the convenient middle
...
\code{end}~$\leftrightarrow$~\code{\fa{پایان}}. A full table of these
...
\begin{note}
Salam gives these two so often that it provides short aliases: \code{\_} is the
same as \code{print}, and \code{\_\_} is the same as \code{println}. So
\code{\_ "hi"} and \code{print "hi"} are identical, and \code{\_\_ "hi"} equals
\code{println "hi"}. You will meet the aliases in some of the bundled examples;
this book prefers the spelled-out names for clarity.
\end{note}

\begin{deep}[In Depth: other ways to write the same thing]
Salam is flexible about a few surface details. Blocks may be written with a
colon and \code{end}, as in this book, \emph{or} with curly braces:
\begin{salam}
func main { println("Hello, World!") }
\end{salam}
Likewise, a call's arguments may be wrapped in parentheses
(\code{println("hi")}) or not (\code{println "hi"}). The two styles are
equivalent; the colon/\code{end} form with bare arguments is the style used by
most of the examples that ship with Salam, so it is the style we adopt here. Pick
one and be consistent and remember the \code{salam fmt} command, which
reformats your code into the canonical style automatically.
\end{deep}
