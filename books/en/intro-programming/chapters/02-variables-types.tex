...
\code{=} and an \emph{initial value}. So \code{age i32 = 30} means ``make a
variable called \code{age}, of type \code{i32} (a 32-bit integer), holding
\code{30}.''
...
\begin{itemize}[leftmargin=1.4em]
  \item \code{const} fixed, and computed at compile time. Use for true
        constants. Conventionally written in \code{UPPER\_CASE}.
  \item plain (\emph{bare}) fixed once set at run time, but computed while the
        program runs. This is the default; prefer it.
  \item \code{mut} may be reassigned. Use only when a value really must
        change.
\end{itemize}
...
type, decided once, at compile time. \code{count} above is exactly an \code{i32}
--- you simply did not have to spell it out. You can combine \code{auto} with
\code{mut}: \code{mut total auto = 0} makes a mutable \code{i32}.
...
\code{i32} it is the default and is almost always the right choice. Step up to
...
\begin{note}
Three of these have friendlier alias names: \code{int} is another spelling of
\code{i32}, \code{uint} of \code{u32}, and \code{float} of \code{f32}. They are
exactly the same types the compiler treats \code{int} and \code{i32} as
identical so use whichever reads better to you. This book mostly uses the
explicit \code{i32}.
\end{note}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \code{f32} (alias \code{float}) a 32-bit number; less precise, but
        smaller. Useful for graphics or large arrays where space matters.
\end{itemize}
...
are the language of decisions every \code{if} and every \code{while} in the
...
func main:
    grade char = 'A'
    println grade
    println grade as i32        // the numeric code of 'A'
...
\midrule
whole number      & \code{i32} & \code{0}, \code{42}, \code{-7} \\
...
\begin{warn}
\textbf{A plain whole-number literal is an \code{i32}.} Salam does not silently
shrink it to fit a smaller or unsigned type. So this is an \emph{error}:
\begin{salam}
mut b u8 = 250           // error: cannot assign 'i32' to 'u8'
\end{salam}
The literal \code{250} is an \code{i32}, and Salam will not narrow it for you
without being asked. The fix is an explicit cast with \code{as}:
\begin{salam}
...
\code{user\_count} tells the reader far more than \code{n} or \code{i32val}. The
...
\midrule
\code{i32} & \code{\fa{صحیح۳۲}} \\
\code{i64} & \code{\fa{صحیح۶۴}} \\
\code{f64} & \code{\fa{اعشاری۶۴}} \\
...
Notice that the digits inside \code{\fa{صحیح۳۲}} are Persian digits
(\fa{۳۲}), while the value \code{30} is written with ordinary digits, because
arithmetic uses the standard digit characters. We will untangle this fully in
Chapter~22; for now, just admire that the structure is identical to the English
version.
...
\code{i32} becomes \code{int32\_t}, \code{f64} becomes \code{double},
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item Integers come in signed (\code{i8}--\code{i64}) and unsigned
        (\code{u8}--\code{u64}) sizes; \code{i32} is the everyday default.
        \code{int}/\code{uint}/\code{float} are aliases for
        \code{i32}/\code{u32}/\code{f32}.
  ...
  \item A whole-number literal is an \code{i32}; conversions are never implicit ---
        use \code{as}. Integer division truncates.
  \item Types have Persian names too (\code{\fa{صحیح۳۲}}, \code{\fa{رشته}},
        \dots).
\end{itemize}
...
count (\code{i32}), and a price (\code{f64}). Print them in one sentence.

\exercise Start with \code{mut total i32 = 0}, add \code{17} and then \code{25}
