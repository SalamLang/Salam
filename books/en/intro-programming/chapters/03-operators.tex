...
Salam has a dedicated exponentiation operator, \code{\textasciicircum\textasciicircum}. It raises the left
...
\begin{note}
Because \code{\textasciicircum\textasciicircum} always yields a floating-point number, \code{2 \textasciicircum\textasciicircum 10} is the
...
\code{x /= y}  & \code{x = x / y} \\
\code{x \%= y} & \code{x = x \% y} \\
\code{x \textasciicircum\textasciicircum= y} & \code{x = x \textasciicircum\textasciicircum y} \\
\bottomrule
\end{tabular}
\end{center}
...
Because \code{\textasciicircum\textasciicircum} always produces a floating-point result (see the previous
section), \code{x \textasciicircum\textasciicircum= y} only type-checks when \code{x} itself is a \code{f64}
(or a \code{f32}, or a type with an \code{operator \textasciicircum\textasciicircum} overload from
Chapter~13) for the same reason \code{x = x \textasciicircum\textasciicircum y} would need \code{x} to accept
...
acts first. The rules follow ordinary mathematics: \code{\textasciicircum\textasciicircum} binds tightest, then
...
\begin{itemize}[leftmargin=1.4em]
  \item Arithmetic: \code{+ - * / \%}, plus \code{\textasciicircum\textasciicircum} for powers (which yields a
  ...
  \item Compound assignment (\code{+=}, \code{-=}, \code{*=}, \code{/=},
        \code{\%=}, \code{\textasciicircum\textasciicircum=}) updates a \code{mut} variable in place, and is
...
\exercise Given \code{mut side : f64 = 3.0}, use \code{\textasciicircum\textasciicircum=} to compute the cube
