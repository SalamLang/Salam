...
\begin{salam}
func greet(name str, greeting str = "Hello", times i32 = 1):
    repeat times:
...
func bump(x i32, by i32 = STEP) i32:
    ret x + by
end
...
after it must have one too. \code{(a i32, b i32 = 0)} is fine, but
\code{(a i32 = 0, b i32)} is rejected if you passed a single argument, the
compiler could not tell which parameter you meant. The default value must also be
assignable to the parameter's type, so \code{by i32 = "five"} is a compile-time
error.
\end{warn}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item Parameters are matched by position and passed \emph{by value} (the
        ...
        value} (\code{b i32 = 0}) that the caller can omit; there are no reference
        parameters.
...
\exercise Write \code{power(base i32, exp i32 = 2) i32} that returns
