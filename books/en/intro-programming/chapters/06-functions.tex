...
\code{square} takes one \textbf{parameter}, \code{n} of type \code{int}, and its
\textbf{return type} (written after the parameter list) is also \code{int}. The
...
A parameter may declare a \textbf{default value} with \code{=}. When the caller
omits that argument, the default is supplied automatically. This lets one
function serve both the common case and the occasional override, without writing
a separate overload:

\begin{salam}
func greet(name : str, greeting : str = "Hello", times : int = 1):
    repeat times:
...
func main:
    greet("Sara")              // greeting = "Hello", times = 1
    greet("Ali", "Hi")         // times = 1
    greet("Max", "Hey", 3)     // everything supplied
end
\end{salam}
...
A default expression is evaluated \emph{at the call site} every time the argument
is omitted, so it may refer to constants and other top-level names but not to the
function's own other parameters:

\begin{salam}
const STEP : int = 5

func bump(x : int, by : int = STEP) : int:
    ret x + by
end

func main:
    println bump(10)       // by = STEP = 5
    println bump(10, 1)    // by = 1
end
\end{salam}
...
\begin{warn}
Defaults must be \emph{trailing}: once a parameter has a default, every parameter
after it must have one too. \code{(a int, b int = 0)} is fine, but
\code{(a int = 0, b int)} is rejected if you passed a single argument, the
compiler could not tell which parameter you meant. The default value must also be
assignable to the parameter's type, so \code{by int = "five"} is a compile-time
error.
\end{warn}

\begin{note}
Default parameters and overloading solve overlapping problems. Reach for a default
when the variants differ only by an omitted value of the \emph{same} type; reach
for an overload (next sections) when they differ by the \emph{type} or
\emph{number} of arguments in a way a single signature cannot express.
\end{note}
...
func main:
    println add(2, 3)        // calls the int version
...
\begin{salam}
func ignore_second(a : int, _b : int) : int:
    ret a
end
\end{salam}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item Parameters are matched by position and passed \emph{by value} (the
        function gets a copy). Trailing parameters may declare a \emph{default
        value} (\code{b int = 0}) that the caller can omit; there are no reference
        parameters.
...
\exercise Write a function \code{cube(n int) int} that returns $n^3$ using
...
\exercise Write \code{min2(a int, b int) int} and use it to build
\code{min3(a, b, c)} by calling \code{min2} twice.

\exercise Write a recursive function \code{sum\_to(n int) int} that returns
...
\exercise Write \code{power(base int, exp int = 2) int} that returns
\code{base} raised to \code{exp}. Call it as \code{power(5)} (should give 25) and
\code{power(2, 10)} (should give 1024). Then try adding a parameter \emph{without}
a default after \code{exp} and read the compiler error.
