...
The type \code{int[5]} reads ``an array of five \code{int}s.'' The values are
...
The \code{each} loop and arrays are made for each other: it walks every element
in turn without any bookkeeping about indices, so the body can simply use each
value as it arrives. Here we sum an array and find its largest value in a single
pass:

\begin{salam}
func main:
    a : int[7] = [5, 9, 1, 7, 3, 8, 2]
    mut sum : int = 0
    mut hi : int = a[0]
    each x in a:
        sum += x
...
The \code{each} loop visits every slot of \code{a} exactly once, in order, and
binds \code{x} to the element's value each time round handing you the value
directly means there is no index to get off by one, and no bound to remember.
When you do need the index alongside the value say, to print ``slot 3 holds
7'' use the two-variable form \code{each (i, x) in a:}.

\section{Multi-dimensional arrays}

An array's elements can themselves be arrays, giving you a grid. \code{int[2][3]}
is ``2 rows of 3 columns.'' You index with two pairs of brackets, row first:

\begin{salam}
func main:
    mut grid : int[2][3] = [[1, 2, 3], [4, 5, 6]]
    grid[1][2] = 60
...
parameter type, because a function that takes \code{int[:]} can be handed
\emph{any} run of \code{int}s: a whole array, or just a piece of one.

\begin{salam}
func total(view : int[:]) : int:
    mut sum : int = 0
    each x in view:
        sum = sum + x
    end
...
\begin{salam}
func main:
    mut nums : int[6] = [10, 20, 30, 40, 50, 60]
...
The angle brackets in \code{Vector<int>} say what the vector holds here,
\code{int}s. (This is your first glimpse of \emph{generics}, the subject of
...
Because \code{.len()} can change, the simplest way to walk a vector is an
\code{each} loop it visits however many elements happen to be there, with no
length to track by hand:
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item Walk an array's elements with \code{each x in a:} (or
        \code{each (i, x) in a:} when you need the index too).
...
\exercise Given \code{a int[6] = [4, 8, 15, 16, 23, 42]}, compute and print the
...
\exercise Fill a \code{int[3][3]} grid so that \code{grid[r][c] = r * 3 + c}, then
