...
You met \code{Vector<i32>} back in Chapter~7 and the bound \code{<T: Shape>} in
...
\code{f64} in place of \code{i32}, and again for any other comparable type. That
...
\code{i32} for the first call and \code{f64} for the second. The two parameters
...
be both \code{i32} and \code{f64} at once.
...
\code{Box<i32>} is a box of integers; \code{Box<str>} is a box of strings. You
spell out the concrete type in the variable's declaration \code{Box<i32>} ---
...
explicitly: \code{Box<i32>}, \code{Vector<str>}. The struct's type parameter
...
Declare \code{Stack<i32>} and you get an integer stack; declare \code{Stack<str>}
...
\code{Max\_f64}; \code{Stack<i32>} becomes a concrete C struct with a
\code{i32[16]} field. Those copies are ordinary, fully type-checked code, as fast
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item A generic struct (\code{Box<T>}, \code{Stack<T>}, \code{Vector<T>}) is
        declared with its concrete type, e.g. \code{Stack<i32>}.
