...
Here \code{(x int) => x * 2} is a function value: it takes an \code{int} called
...
The type of \code{double} is \code{func(int) int}: ``a function taking an
\code{int} and returning an \code{int}.'' In general a function-value type is
written \code{func(}\textit{argument types}\code{) }\textit{return type}. A
function value taking nothing and returning an \code{int} has type
\code{func() int}. You will see these types most often where one function accepts
...
\code{func(int) int}, you can declare a parameter of that type and call it inside:
...
We hand it a lambda created right at the call site, \code{(x int) => x + 100}, and
...
\begin{itemize}[leftmargin=1.4em]
  \item A \textbf{lambda} is an unnamed function value:
        ...
        block body.
  \item A function value has a type like \code{func(int) int} or \code{func()
        int}.
...
\exercise Write a higher-order function \code{apply\_twice(f func(int) int, x
int) int} that returns \code{f(f(x))}, and test it with a lambda that adds 3.

\exercise Write \code{make\_multiplier(factor int) func(int) int} that returns a
...
\code{Vector<int>} and a \code{func(int) int}, and returns a new vector with the
