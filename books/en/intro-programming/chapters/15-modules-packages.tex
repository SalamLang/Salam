...
You can write one \code{import} per module, or group them in a single
parenthesised block the grouped form is tidy when a file uses several modules:
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item Group several imports in one \code{import ( ... )} block; give any import
        an \code{alias} by writing the alias before the name.
...
\exercise Write a program that imports \code{math} and \code{str} in a grouped
\code{import} block and prints the square root of 2 alongside the word
