...
its return type. You do this in an \code{extern} block, which lists the
...
Inside the \code{extern} block, each \code{func} line is a \emph{declaration}:
...
\midrule
\code{i32} (\code{int}) & \code{int32\_t} & a 32-bit signed integer \\
...
\code{link dynamic "sqlite3"} instructs the compiler to dynamically link against
the SQLite library (the equivalent of passing \code{-lsqlite3} to a C compiler),
making its functions available to your \code{extern} declarations. You can link
several libraries with several \code{link} lines. The kind before the library
name is not optional there is no bare \code{link "x"}: write \code{dynamic} for
the ordinary shared-library case shown here, or \code{static} to force a static
archive (\code{-l:libx.a}), or (macOS only) \code{framework} for a
\code{.framework} bundle.

\begin{deep}[In Depth: platform-conditional linking]
Library names sometimes differ across operating systems. Salam has no separate
preprocessor syntax for this; it reuses the ordinary \code{if}/\code{else}/\code{end}
statement at the top level, folded at compile time, to choose the right one at
build time:
\begin{salam}
if SALAM_OS_WINDOWS:
    link dynamic "winsqlite3"
else:
    link dynamic "sqlite3"
end
\end{salam}
...
Salam. Unlike a runtime \code{if}, the untaken branch is never even type-checked.
\end{deep}
...
\begin{itemize}[leftmargin=1.4em]
  \item An \code{extern:} block declares C functions (name, parameters,
  ...
  \item Standard-library C functions need only declaration; others need a
        \code{link dynamic "lib"} (or \code{static}/\code{framework}) directive
        (optionally guarded by \code{@if} for per-platform names).
...
\exercise Declare C's \code{puts} (\code{func puts(s str) int}) and use it to print
two lines.
