...
\begin{note}
\code{join} is how you know a thread is done; a program should \code{join} every
thread it spawns before relying on that thread's results.
\end{note}

\section{Giving a thread its own data}

A thread started as \code{spawn(greet)} can only reach shared variables, which
means every worker sees exactly the same thing. The second form of \code{spawn}
hands the new thread one value of its own:
...
func main:
    a := mem.Allocate(sizeof(Job) as u64) as Job*
    a[0] = Job { id = 7 }
    t := spawn(work, a)
    join(t)
    mem.Free(a as void*)
...
The worker's single parameter must be a \emph{pointer}, because the value travels
to the new thread through the operating system's one thread argument slot, and a
pointer is the only thing guaranteed to arrive unchanged. That is also why the
example allocates the \code{Job} rather than using a local: the memory has to
outlive the \code{spawn} call, and here \code{join} guarantees it does.
