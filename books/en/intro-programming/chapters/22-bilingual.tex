...
fixed words like \code{func}, \code{if}, \code{repeat}) and \emph{your} names (the
...
\code{until}  & \code{\fa{تاوقتی}}  & \code{each}    & \code{\fa{هر}} \\
...
\code{interface} & \code{\fa{واسط}} & \code{end}     & \code{\fa{تمام}} \\
...
\textbf{English} & \textbf{Persian} & \textbf{English} & \textbf{Persian} \\
\midrule
\code{int} & \code{\fa{صحیح}} & \code{i64} & \code{\fa{صحیح۶۴}} \\
\code{f64} & \code{\fa{اعشار۶۴}} & \code{bool} & \code{\fa{منطقی}} \\
...
\begin{note}
Notice the digits. When a Persian type name carries a width, it carries it in
Persian digits: \code{\fa{صحیح۶۴}} is \code{i64}, and the \fa{۶۴} inside it is
part of the keyword's \emph{spelling}. Arithmetic values such as \code{15} or loop
bounds such as \code{5}, by contrast, are written with the ordinary digits
\code{0}--\code{9}. Spelling may use Persian digits; computation uses the standard
ones.
\end{note}
