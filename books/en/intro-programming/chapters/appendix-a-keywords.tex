...
\code{until}      & \code{\fa{تاوقتی}}      & loop while a condition holds \\
\code{each}       & \code{\fa{هر}}          & iterate the elements of a collection \\
\code{in}         & \code{\fa{در}}          & collection to iterate (with \code{each}) \\
\code{with}       & \code{\fa{با}}          & bind an index variable (with \code{each}/\code{repeat}) \\
\code{repeat}     & \code{\fa{تکرار}}       & repeat / range loop \\
\code{to}         & \code{\fa{تا}}          & range upper bound (with \code{repeat}) \\
\code{by}         & \code{\fa{گام}}         & range step (with \code{repeat ... to}) \\
...
\code{on}         & \code{\fa{بر}}          & interface target (in \code{impl ... on ...}) \\
\code{end}        & \code{\fa{تمام}}       & close a block \\
...
\begin{note}
The entry point \code{main} / \code{\fa{اصلی}} is part of the language:
