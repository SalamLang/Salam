...
Operators: \code{+ - * / \%}, \code{\textasciicircum\textasciicircum} (power, float), \code{== != < > <= >=},
\code{\&\& || !}, \code{+= -= *= /= \%= \textasciicircum\textasciicircum=}. \\
...
\begin{salam}
print s  println s  io.Input()  io.ReadFile(p)  io.WriteFile(p, t)
...
link dynamic "sqlite3"                // link an external library (or static/framework)
\end{salam}
...
t2: i64 = spawn(work_on, job)          // ... with one pointer argument
join(t)                               // wait for it
...
\code{salam format f.salam} & reformat source in place \\
