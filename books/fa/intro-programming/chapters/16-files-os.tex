...
یک \textbf{پیش‌پردازنده} دارد که اجازه می‌دهد بر اساسِ سیستم‌عامل، بخش‌هایی از
کد را در زمانِ کامپایل انتخاب کنید. این مثالِ واقعی را ببینید:
...
دستورهای \code{@if}، \code{@else} و \code{@end} \emph{پیش} از کامپایلِ اصلی
اجرا می‌شوند و فقط شاخهٔ درست را نگه می‌دارند. پرچم‌هایی مانندِ
\code{SALAM\_OS\_WINDOWS}، \code{SALAM\_OS\_MAC} و \code{SALAM\_OS\_UNIX} را
خودِ کامپایلر بر اساسِ سیستمِ مقصد تعیین می‌کند.

\begin{deep}[نگاهی به درون: تفاوتِ پیش‌پردازنده و شرطِ معمولی]
چه فرقی است میانِ \code{@if} و \code{اگر}؟ \code{اگر} در \emph{زمانِ اجرا}
تصمیم می‌گیرد و هر دو شاخه در برنامهٔ نهایی حضور دارند. اما \code{@if} در
\emph{زمانِ کامپایل} تصمیم می‌گیرد و شاخهٔ ناخواسته اصلاً واردِ برنامهٔ نهایی
...
گذاشتنِ پیام‌های اشکال‌زدایی که فقط در نسخهٔ توسعه فعال‌اند.
...
\begin{itemize}
  ...
  \item پیش‌پردازنده (\code{@if}/\code{@else}/\code{@end}) کد را بر اساسِ
        سیستم‌عامل در زمانِ کامپایل انتخاب می‌کند.
\end{itemize}
