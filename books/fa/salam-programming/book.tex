...
% ---------------------------------------------------------------------
% اصلاح دو مشکل ماکروهای مشترک کارت کد برای متن راست‌به‌چپ:
% ۱) توکن‌های تمام‌لاتین (عدد، عملگر، رشته‌ی انگلیسی) باید چپ‌به‌راست و
%    بیرون از حالت فارسی حروف‌چینی شوند، وگرنه xepersian ترتیب رقم‌ها و
%    علامت‌ها را وارونه می‌کند («3.5,» به «,5.3»). این‌جا هر توکنی که فقط
%    نویسه‌ی اسکی دارد در \lr و با قلم کد لاتین چیده می‌شود.
...
\newcommand{\salamfa@tok}[1]{\salamfa@asciitrue\salamfa@asciiloop#1\relax
  \ifsalamfa@ascii\lr{{\latincodefont #1}}\else#1\fi}
\renewcommand{\salamfa@classify}[1]{%
  \ifsalamfa@linecmt
    \salamfa@gadd{\cmtword{\salamfa@tok{#1}}}%
  \else\ifsalamfa@incmt
    \salamfa@gadd{\cmtword{\salamfa@tok{#1}}}%
    \IfBeginWith{#1}{/*}{\global\advance\salamfa@cmtdepth\@ne}{}%
    \IfEndWith{#1}{*/}{\salamfa@closecmt}{}%
  \else\ifsalamfa@instr
    \salamfa@gadd{\stword{\salamfa@tok{#1}}}%
    \IfSubStr{#1}{"}{\salamfa@instrfalse}{}% chktex 18
  \else
    \IfBeginWith{#1}{//}%
      {\salamfa@linecmttrue \salamfa@gadd{\cmtword{\salamfa@tok{#1}}}}%
  ...
  \IfBeginWith{#1}{"}% chktex 18
    {\StrGobbleLeft{#1}{1}[\salamfa@rest]%
     \expandafter\IfSubStr\expandafter{\salamfa@rest}{"}% chktex 18
       {\salamfa@gadd{\stword{\salamfa@tok{#1}}}}%
       {\salamfa@gadd{\stword{\salamfa@tok{#1}}}\salamfa@instrtrue}}%
    {\ifcsname salamkw@#1\endcsname
     ...
     \else
       ...
       \ifcsname salamfa@dig@\salamfa@fc\endcsname
         \salamfa@gadd{\textcolor{nm}{\salamfa@tok{#1}}}%
       \else
         \salamfa@gadd{\salamfa@tok{#1}}%
       \fi
     \fi\fi}}
\makeatother
