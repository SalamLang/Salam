...
if __name__ == "__main__":
    ...
    element.add_attribute().set_generate_name("autofocus").set_type(
        Type.Boolean
    ).add_text(Lang.EN, "focus").add_text(Lang.FA, "فوکوس")
    ...
    element.add_attribute().set_generate_name("formenctype").set_type(
        Type.String
    ).add_text(Lang.EN, "formenctype").add_text(Lang.FA, "نوع فرم").add_reserve_values(
        mediatypes.mediatypes
    )
    element.add_attribute().set_generate_name("formmethod").set_type(
        Type.Boolean
    ).add_text(Lang.EN, "method").add_text(Lang.FA, "شیوه").add_reserve_values(
        methods.methods
    )
    # TODO
    element.add_attribute().set_generate_name("formnovalidate").set_type(
        Type.Boolean
    ).add_text(Lang.EN, "formnovalidate").add_text(
        Lang.FA, "بدون اعتبارسنجی فرم"
    ).add_reserve_values(
        targets.targets
    )
    # TODO
    element.add_attribute().set_generate_name("formtarget").set_type(
        Type.String
    ).add_text(Lang.EN, "formtarget").add_text(Lang.FA, "فرم هدف").add_reserve_values(
        targets.targets
    )
    # TODO
    element.add_attribute().set_generate_name("popovertarget").set_type(
        Type.ID
    ).add_text(Lang.EN, "popovertarget").add_text(Lang.FA, "پاپ روی هدف")
    # TODO
    element.add_attribute().set_generate_name("popovertargetaction").set_type(
        Type.AnyNoEmpty
    ).add_text(Lang.EN, "popovertargetaction").add_text(
        Lang.FA, "popovertargetaction"
    ).add_reserve_values(
        [
            Value()
            .set_generate_name("hide")
            .add_text(Lang.EN, "hide")
            .add_text(Lang.FA, "مخفی"),
            Value()
            .set_generate_name("show")
            .add_text(Lang.EN, "show")
            .add_text(Lang.FA, "نمایش"),
            Value()
            .set_generate_name("toggle")
            .add_text(Lang.EN, "toggle")
            .add_text(Lang.FA, "متغیر بودن"),
        ]
    )
    ...
    element.add_attribute().set_generate_name("type").set_type(
        Type.AnyNoEmpty
    ).add_text(Lang.EN, "type").add_text(Lang.FA, "نوع").add_reserve_values(
        [
            Value()
            .set_generate_name("button")
            .add_text(Lang.EN, "button")
            .add_text(Lang.FA, "دکمه"),
            Value()
            .set_generate_name("reset")
            .add_text(Lang.EN, "reset")
            .add_text(Lang.FA, "ریست"),
            Value()
            .set_generate_name("submit")
            .add_text(Lang.EN, "submit")
            .add_text(Lang.FA, "ارسال"),
        ]
    )
    element.add_attribute().set_generate_name("value").set_type(
        Type.AnyNoEmpty
    ).add_text(Lang.EN, "content").add_text(Lang.FA, "محتوا").set_is_required(True)
