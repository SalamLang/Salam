...
if __name__ == "__main__":
    ...
    element.add_attribute().set_generate_name("dirname").set_type(
        Type.AnyNoEmpty
    ).add_text(Lang.EN, "dirname").add_text(Lang.FA, "نام جهت")
    ...
    element.add_attribute().set_generate_name("max_length").set_type(
        Type.PositiveInt
    ).add_text(Lang.EN, "max length").add_text(Lang.FA, "حداکثر طول")
    ...
    element.add_attribute().set_generate_name("placeholder").set_type(
        Type.String
    ).add_text(Lang.EN, "placeholder").add_text(Lang.FA, "راهنما")
    element.add_attribute().set_generate_name("readonly").set_type(
        Type.Boolean
    ).add_text(Lang.EN, "readonly").add_text(Lang.FA, "فقط خواندنی")
    ...
    element.add_attribute().set_generate_name("wrap").set_type(Type.String).add_text(
        Lang.EN, "wrap"
    ).add_text(Lang.FA, "رپ").add_reserve_values(
        [
            Value()
            .set_generate_name("soft")
            .add_text(Lang.EN, "soft")
            .add_text(Lang.FA, "نرم"),
            Value()
            .set_generate_name("hard")
            .add_text(Lang.EN, "hard")
            .add_text(Lang.FA, "سخت"),
        ]
    )
