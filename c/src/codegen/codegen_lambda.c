...
void cg_emit_lambda(cg_t *cg, ast_node_t *n)
{
    ...
    const char *ps = arena_strdup(cg->a, sb_cstr(&params));
    sb_free(&params);
    ...
    if (ncap) {
        ...
        sb_puts(cg->lam_decls, sb_cstr(&st));
        sb_free(&st);
    ...
    if (ncap) {
        ...
        const char *mps = arena_strdup(cg->a, sb_cstr(&mp));
        sb_free(&mp);
        ...
        cg_line(cg, "e->fn = (void*)&%s;", name);
        {
            ...
            for (; i < ncap; i++) {
                ast_node_t *c = (ast_node_t *)n->captures.data[i];
                cg_line(cg, "e->%s = %s;", cg_cident(cg, c->name),
                        cg_cident(cg, c->name));
            }
        }
        cg_line(cg, "return e;");
...
const char *cg_lambda_value(cg_t *cg, ast_node_t *n)
{
    ...
    if (n->captures.len == 0) return cg_fmt(cg, "(void*)&%s_env", n->name);
    sb_t a;
    sb_init(&a);
    {
        size_t i = 0;
    ...
    const char *args = arena_strdup(cg->a, sb_cstr(&a));
    sb_free(&a);
...
void cg_lift_walk(cg_t *cg, ast_node_t *node)
{
    ...
    if (node->kind == AST_LAMBDA) {
        cg_lift_walk(cg, node->a);
        cg_emit_lambda(cg, node);
        return;
    }
    cg_lift_walk(cg, node->a);
    cg_lift_walk(cg, node->b);
    cg_lift_walk(cg, node->c);
    cg_lift_walk(cg, node->d);
    {
        size_t i = 0;
