...
static bool ts_is_int(const char *ts)
{
                              ...
                              "u16", "u32", "u64", "uint", "size", 0};
    {
        int i = 0;
        for (; k[i]; i++)
            if (!strcmp(ts, k[i])) return true;
    }
    return false;
}

static bool ts_is_float(const char *ts)
{
...
static bool fold_flt(ast_node_t *n, double *out)
{
    long long iv;
    if (!n) return false;
    if (n->kind == AST_LITERAL && n->op == TK_FLOAT) {
        *out = n->value.as.f;
        return true;
    }
    if (fold_int(n, &iv)) {
...
static bool pf_try_fold(ast_node_t *arg, sb_t *sb)
{
    ...
    if (arg->kind == AST_LITERAL) {
        ...
        case TK_RAW_STRING:
            sb_puts(sb, arg->value.as.s ? arg->value.as.s : "");
            return true;
        case TK_UTF8_CHAR:
            sb_puts(sb, arg->value.as.s ? arg->value.as.s : "");
            return true;
        case TK_CHAR:
            append_utf8(sb, (unsigned long)arg->value.as.i);
            return true;
        default:
            break;
        }
    ...
    if (ts_is_int(ts)) {
        ...
        if (fold_int(arg, &v)) {
            ...
            if (ts_is_unsigned(ts))
                sal_u64toa((uint64_t)v, b);
            else
                sal_i64toa((int64_t)v, b);
            sb_puts(sb, b);
            return true;
        }
    }
