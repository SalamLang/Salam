...
static size_t get_src_line(const char *text, size_t text_len, uint32_t lineno, char *buf,
                           size_t cap)
{
    const char *p = text;
...
static void write_diag_code(FILE *sink, bool is_error, int code)
{
    if (strcmp(i18n_lang(), "fa") == 0) {
        static const char *const fa_digit[] = {"۰", "۱", "۲", "۳", "۴",
                                               "۵", "۶", "۷", "۸", "۹"};
...
static void span_visuals(const char *line_buf, size_t line_len, const src_span_t *span,
...
void diag_render_rust(FILE *sink, const char *level, int code, bool is_error,
                      const char *file, const src_span_t *span, const char *src_text,
                      size_t src_len, const char *body, const char *help, bool color)
{
    ...
    if (code > 0) write_diag_code(sink, is_error, code);
    fprintf(sink, "%s: %s%s%s\n", reset, bold, body, reset);
    if (!file) return;

    if (span && span->begin.line > 0)
        fprintf(sink, " %s-->%s %s:%u:%u\n", blue, reset, file, span->begin.line,
                span->begin.col);
    else {
        fprintf(sink, " %s-->%s %s\n", blue, reset, file);
        if (help)
            fprintf(sink, "   %s= %s:%s %s\n", color ? A_GREEN : "", i18n_tr("help"),
                    reset, help);
        return;
    }
    ...
    size_t line_len = 0;
    if (src_text)
        line_len = get_src_line(src_text, src_len, lineno, line_buf, sizeof(line_buf));
                     ...
                     size_t src_len, const char *body, const char *help, bool color)
{
    ...
    if (file && span && span->begin.line > 0)
        fprintf(sink, "%s%s:%u:%u:%s ", bold, file, span->begin.line, span->begin.col,
                reset);
    else if (file)
    ...
    if (!src_text || !span || span->begin.line == 0) {
        if (help)
            fprintf(sink, "  %s%s:%s %s\n", color ? A_CYAN : "", i18n_tr("note"), reset,
                    help);
        return;
    }
    ...
    char line_buf[1024];
    size_t line_len =
        get_src_line(src_text, src_len, span->begin.line, line_buf, sizeof(line_buf));
    fputs("    ", sink);
...
void diag_render_clang(FILE *sink, const char *level, int code, bool is_error,
                       const char *file, const src_span_t *span, const char *src_text,
                       size_t src_len, const char *body, const char *help, bool color)
{
    const char *lc = color ? level_color(level) : "";
    ...
    if (span->begin.col > 0) span_visuals(line_buf, line_len, span, &vis_s, &vis_w);
