...
int32_t mysql_select_db(void *conn, const char *db)
{
    (void)conn;
    (void)db;
    return 0;
...
int32_t mysql_set_character_set(void *conn, const char *cs)
{
    (void)conn;
    (void)cs;
    return 0;
...
uint64_t mysql_num_rows(void *result)
{
    ...
    while (sqlite3_step(r->stmt) == MOCK_SQLITE_ROW)
        n++;
    sqlite3_reset(r->stmt);
    return n;
}
...
{
    return "mock-mariadb (sqlite-backed)";
}

const char *mysql_get_server_info(void *conn)
{
    (void)conn;
    return "mock-server";
}
