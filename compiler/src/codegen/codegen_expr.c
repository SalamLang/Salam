    ...
    sb_putc(&b, '"');
    {
        ...
        for (; *p; p++) {
            ...
            default:
                if (ch < 0x20 || ch >= 0x7f) {
                    char oct[5];
                    sal_snprintf(oct, sizeof(oct), "\\%03o", ch);
                    sb_puts(&b, oct);
                } else {
                    sb_putc(&b, (char)ch);
                }
                            ...
                            const char *sname, const char *mname, ast_node_t *rhs_node)
{
    ...
    if (lhs_is_ptr || addressable) {
        ...
        if (ret_struct) {
            int s = ++cg->tmpn;
            return cg_fmt(cg, "({ %s __s%d; %s(%s%s%s, &__s%d); __s%d; })", Rc, s,
                          mangled, recv, sep, rhs_c ? rhs_c : "", s, s);
        }
    ...
    if (ret_struct) {
        int sv = ++cg->tmpn;
        return cg_fmt(cg,
                      "({ %s __t%d = (%s); %s __s%d; %s(&__t%d%s%s, &__s%d); __s%d; })",
                      cg_ctype(cg, lhs_ts), t, lhs_c, Rc, sv, mangled, t, sep,
                      rhs_c ? rhs_c : "", sv, sv);
    }
    return cg_fmt(cg, "({ %s __t%d = (%s); %s(&__t%d%s%s); })", cg_ctype(cg, lhs_ts), t,
...
const char *cg_expr(cg_t *cg, ast_node_t *n)
{
    ...
    case AST_LITERAL:
        ...
        case TK_FLOAT: {
            ...
            if (!strpbrk(buf, ".eEnN")) return cg_fmt(cg, "%s.0", buf);
        ...
        case TK_RAW_STRING:
        case TK_UTF8_CHAR:
            return cg_cescape(cg, n->value.as.s ? n->value.as.s : "");
            ...
            if (ssym) {
                ...
                if (mname) {
                    ...
                    if (m && m->kind == SYM_METHOD && pick_op_overload(cg, m, 0)) {
                        const char *call =
                            cg_emit_op_call(cg, n->a, ssym, sname, mname, NULL);
                        if (call) return call;
                    }
                }
            }
    ...
    case AST_CAST: {
...

        {
            ...
            if ((dptr != sptr && (cg_is_int_typestr(dts) || cg_is_int_typestr(sts))) ||
                ...
                return cg_fmt(cg, "((%s)(intptr_t)(%s))", cg_ctype(cg, tgt),
                              cg_expr(cg, n->a));
    ...
    case AST_MEMBER: {
        ...
        return cg_fmt(cg, "%s%s%s", cg_expr(cg, n->a), ptr ? "->" : ".",
                      cg_cident(cg, n->name));
    }
    case AST_INDEX: {
        if (n->a && n->a->type_str) {
            ...
            if (ssym) {
                ...
                if (m && m->kind == SYM_METHOD) {
                    const char *call =
                        cg_emit_op_call(cg, n->a, ssym, sname, "operator_index", n->b);
                    if (call) return call;
                }
            }
        }
        if (cg_is_slice_ts(n->a->type_str)) {
            char elem[96];
            cg_slice_elem(n->a->type_str, elem, sizeof elem);
    ...
    case AST_SLICE: {
        char elem[96];
        cg_slice_elem(n->type_str, elem, sizeof elem);
        const char *ec = cg_ctype(cg, elem);
