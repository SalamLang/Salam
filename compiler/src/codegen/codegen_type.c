...
{
    kbuf[0] = vbuf[0] = 0;
    const char *lt = strchr(ts, '<'); if (!lt) return;
    const char *comma = strchr(lt, ',');
    const char *gt = strrchr(ts, '>');
    if (!comma || !gt) return;
    const char *ks = lt + 1; while (*ks == ' ') ks++;
    const char *ke = comma;  while (ke > ks && ke[-1] == ' ') ke--;
    const char *vs = comma + 1; while (*vs == ' ') vs++;
    const char *ve = gt;     while (ve > vs && ve[-1] == ' ') ve--;
    size_t kl = (size_t)(ke - ks); if (kl >= cap) kl = cap - 1; memcpy(kbuf, ks, kl); kbuf[kl] = 0;
...
void parse_typestr(const char *ts, char *base, size_t cap, bool *ptr, vec_t *dims, arena_t *a)
{
    *ptr = false;
    ...
    memcpy(base, ts, blen); base[blen] = 0;
}
...
{
    ebuf[0] = 0;
    const char *lt = strchr(ts, '<'); const char *gt = strrchr(ts, '>');
    if (!lt || !gt || gt <= lt) return;
    const char *es = lt + 1; while (*es == ' ') es++;
    const char *ee = gt;     while (ee > es && ee[-1] == ' ') ee--;
    size_t el = (size_t)(ee - es); if (el >= cap) el = cap - 1;
...
const char *cg_decl(cg_t *cg, const char *ts, const char *name)
{
    if (!strncmp(ts, "dyn ", 4)) {
        ...
        return cg_fmt(cg, "_Salam_dyn_%s%s %s%s", cg_cident(cg, iface), star, cg_cident(cg, name), dims);
    }
    ...
    { const char *p = s; for (; ; p++) {
        ...
        if ((*p==',' && depth==0) || (*p==')' && depth==0) || *p=='\0') {
            ...
            if (len) {
                ...
                if (any) sb_puts(&out, ", ");
                sb_puts(&out, cg_ctype(cg, buf)); any = true;
            }
            if (*p==')' || *p=='\0') break;
            start = p + 1;
        }
