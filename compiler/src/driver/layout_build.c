...
int driver_layout_build(options_t *opt)
{
    ...
    if (!pack) { LOG_E(log, PH_DRIVER, "unknown language pack '%s'", opt->lang);
    ...
    { int i = 0; for (; i < n; i++) {
        ...
        if (opt->inline_mode) {
            ...
            write_file(log, html_path, layout_document(arena, r, false, "style.css", "script.js"));
            ...
            write_file(log, html_path, layout_document(arena, r, false, css_href, js_href));
