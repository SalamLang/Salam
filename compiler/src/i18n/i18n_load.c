...
static const i18n_pair_t k_value_fa[] = {{NULL, NULL}};

static const i18n_pair_t k_layout_fa[] = {{NULL, NULL}};

static struct {
    const char *lang, *alias, *canon;
} g_dyn_vals[I18N_DYN_VAL_MAX];
...
const char *i18n_layout_word(const char *name)
{
    ...
    if (strcmp(lang, "fa") == 0) {
        {
            const i18n_pair_t *p = k_layout_fa;
            for (; p->key; p++)
...
const char *i18n_layout_value(const char *value)
{
    ...
    if (strcmp(lang, "fa") == 0) {
        {
            const i18n_pair_t *p = k_value_fa;
            for (; p->key; p++)
