...
static bool is_intlike(value_t v)
{
    return v.kind == VAL_INT || v.kind == VAL_CHAR;
}

static bool is_number(value_t v)
{
    return v.kind == VAL_INT || v.kind == VAL_CHAR || v.kind == VAL_FLOAT;
}
...
static void utf8_put(sb_t *b, int64_t cp)
{
    ...
    if (c < 0x80) {
        ...
        sb_putc(b, (char)(0xC0 | (c >> 6)));
        sb_putc(b, (char)(0x80 | (c & 0x3F)));
    } else if (c < 0x10000) {
        sb_putc(b, (char)(0xE0 | (c >> 12)));
        sb_putc(b, (char)(0x80 | ((c >> 6) & 0x3F)));
        sb_putc(b, (char)(0x80 | (c & 0x3F)));
    } else {
        sb_putc(b, (char)(0xF0 | (c >> 18)));
        sb_putc(b, (char)(0x80 | ((c >> 12) & 0x3F)));
        sb_putc(b, (char)(0x80 | ((c >> 6) & 0x3F)));
        sb_putc(b, (char)(0x80 | (c & 0x3F)));
    }
}
...
const char *to_str(interp_t *I, value_t v)
{
    ...
    case VAL_CHAR: {
        ...
        sb_init(&b);
        utf8_put(&b, v.as.i);
        const char *r = arena_strdup(I->a, sb_cstr(&b));
        sb_free(&b);
        return r;
    }
    case VAL_BOOL:
    ...
    case VAL_STRUCT: {
        sb_t b;
        sb_init(&b);
        sb_puts(&b, v.as.st->type_name);
        sb_puts(&b, "{");
        {
            size_t i = 0;
            for (; i < v.as.st->nfields; i++) {
                if (i) sb_puts(&b, ", ");
                sb_puts(&b, v.as.st->fields[i].name);
                sb_puts(&b, "=");
...
void map_put(interp_t *I, smap_t *m, value_t k, value_t val)
{
    ...
    if (m->count + 1 > m->cap) {
        ...
        smap_entry_t *ne =
            (smap_entry_t *)arena_alloc(I->a, salam_size_mul(nc, sizeof(smap_entry_t)));
...
value_t mk_struct(interp_t *I, const char *name, ast_node_t *def, size_t nfields)
{
    ...
    s->nfields = nfields;
    s->fields =
        nfields ? (sfield_t *)arena_alloc(I->a, salam_size_mul(nfields, sizeof(sfield_t)))
                : NULL;
    value_t v;
    v.kind = VAL_STRUCT;
    v.as.st = s;
...
bool is_int_typename(const char *b)
{
                                 ...
                                 "u64", "int", "byte", "char", "isize", "usize", NULL};
    {
        int i = 0;
        for (; ints[i]; i++)
            if (strcmp(b, ints[i]) == 0) return true;
    }
    return false;
}
...
value_t arith(interp_t *I, ast_node_t *n, token_kind_t op, value_t a, value_t b)
{
    ...
    case TK_STAR:
        return val_float(x * y);
    case TK_SLASH:
        if (y == 0.0) rt_error(I, n, "division by zero");
        return val_float(x / y);
    case TK_PERCENT:
        if (y == 0.0) rt_error(I, n, "modulo by zero");
