...
static bool is_include_attr(const char *n)
{
    return name_eq(n, "include") || name_eq(n, "بگنجان");
}
...
static const char *node_text(expand_t *ex, ast_node_t *v)
{
    ...
    if (v->kind == AST_LITERAL) {
        ...
        case TK_INT:
            sal_u64toa((uint64_t)v->value.as.i, buf);
            return arena_strdup(ex->a, buf);
        case TK_FLOAT:
            sal_snprintf(buf, sizeof(buf), "%g", v->value.as.f);
            return arena_strdup(ex->a, buf);
        case TK_KW_TRUE:
            return "true";
        case TK_KW_FALSE:
...
static ast_node_t *subst_value(expand_t *ex, ast_node_t *v, frame_t *frame)
{
    ...
    if (v->kind == AST_LITERAL && (v->op == TK_STRING || v->op == TK_TRIPLE_STRING) &&
        v->value.as.s) {
...
{
    {
        ...
        for (; i < program->list.len; i++) {
...

            {
                ...
                for (; j < d->list.len; j++) {
                    ...
                    if (p->kind == AST_PARAM && is_content_word(p->name))
                        EXERR(ex,
                              "component '%s': 'content' is reserved and cannot be a "
                              "parameter",
                              d->name);
                }
            }
            ...
            LOG_D(ex->log, PH_DRIVER, "registered component '%s' (%s)", d->name,
                  c->mother ? "mother" : "single");
        }
    }
}
...
{
    {
        size_t i = 0;
        for (; i < program->list.len; i++)
            if (((ast_node_t *)program->list.data[i])->kind == AST_LAYOUT_COMPONENT)
                return true;
    }
    return false;
}
...
static void scan_includes(expand_t *ex, ast_node_t *container, const char *dir,
                          int depth);
    ...
    register_components(ex, program, dir);
    {
        ...
        for (; i < program->list.len; i++) {
            ...
            if (d->kind == AST_LAYOUT_BLOCK)
                scan_includes(ex, d, dir, depth);
            else if (d->kind == AST_LAYOUT_COMPONENT)
                scan_includes(ex, d->a, dir, depth);
        }
    }
}
...
static void expand_items(expand_t *ex, vec_t *out, ast_node_t *container, frame_t *frame,
                         const char *dir, int depth);
static void expand_item(expand_t *ex, vec_t *out, ast_node_t *item, frame_t *frame,
                        const char *dir, int depth);
                             ...
                             comp_t *comp, frame_t *caller, const char *dir, int depth)
{
    ...
    {
        size_t i = 0;
        for (; i < comp->def->list.len; i++) {
            ast_node_t *param = (ast_node_t *)comp->def->list.data[i];
            if (param->kind == AST_PARAM) frame_set(ex, &fr, param->name, param->a);
        }
    }
    ...
    if (!comp->mother && got_content)
        EXERR(ex,
              "component '%s' does not accept content (it has no `content:` placeholder)",
              comp->name);
                        ...
                        const char *dir, int depth)
{
    if (item->kind == AST_LAYOUT_ATTR) {
        if (is_include_attr(item->name)) {
            ...
            const char *spec =
                (val && val->kind == AST_LITERAL && val->value.kind == TV_STRING)
                    ? val->value.as.s
                    : NULL;
            if (!spec) {
                EXERR(ex, "include path must be a string");
                return;
            }
            const char *path = salam_resolve_import(ex->a, dir, spec);
            ...
            if (l && l->prog) {
                ...
                if (lb) {
                    ...
                    LOG_W(ex->log, PH_DRIVER, "included file '%s' has no layout block",
                          path);
                }
            }
            return;
    ...
    if (item->kind == AST_LAYOUT_ELEMENT) {
        ...
        if (comp) {
            expand_component(ex, out, item, comp, frame, dir, depth);
            return;
        }
                         ...
                         const char *dir, int depth)
{
    ...
    }
    {
        size_t i = 0;
        for (; i < container->list.len; i++)
            expand_item(ex, out, (ast_node_t *)container->list.data[i], frame, dir,
                        depth);
    }
}
