    ...
    long body_repeat = 1;
    {
        ...
        for (; i < layout_block->list.len; i++) {
            ...
            if (!strcmp(item->name, "title"))
                cx.title = v;
            else if (!strcmp(item->name, "lang")) {
                const char *l = layout_map_lang(i18n_layout_value(v));
                if (l) cx.lang = l;
            } else if (!strcmp(item->name, "dir")) {
                const char *d = layout_map_dir(i18n_layout_value(v));
                if (d) cx.dir = d;
            } else if (!strcmp(item->name, "repeat")) {
                if (item->a && item->a->kind == AST_LITERAL &&
