                   ...
                   const char *parent_class)
{
    long n = repeat_count(el);
    {
        long i = 0;
        for (; i < n; i++)
            gen_element(cx, el, parent, parent_class);
    }
}
                        ...
                        const char *parent_class)
{
    ...
    if (!def)
        LWARN(cx, 2, &el->span, "unrecognized layout element '%s' (rendered as <div>)",
              el->name);
    ...
    if (!strcmp(el->name, "heading")) {
        long sz = 1;
        {
            ...
            for (; i < el->list.len; i++) {
                ...
                if (attr->kind == AST_LAYOUT_ATTR && attr->name &&
                    !strcmp(attr->name, "size") && attr->a &&
                    attr->a->kind == AST_LITERAL && attr->a->value.kind == TV_INT)
    ...
    bool needs_class =
        has_css || hover.len || focus.len || active.len || before.len || after.len;
    const char *klass = userclass;
    if (needs_class && !klass) {
        sb_t k;
        sb_init(&k);
        sb_puts(&k, sb_cstr(&css));
        sb_putc(&k, '\x1f');
        sb_puts(&k, sb_cstr(&hover));
        sb_putc(&k, '\x1f');
        sb_puts(&k, sb_cstr(&focus));
        sb_putc(&k, '\x1f');
        sb_puts(&k, sb_cstr(&active));
        sb_putc(&k, '\x1f');
        sb_puts(&k, sb_cstr(&before));
        sb_putc(&k, '\x1f');
        sb_puts(&k, sb_cstr(&after));
    ...
    if (!strcmp(el->name, "image")) {
        html_line(cx, "<img src=\"%s\"%s%s alt=\"%s\" />",
                  source ? html_escape(cx, source) : "", classattr, A, esc);
        goto done;
    }
    if (!strcmp(el->name, "input")) {
        if (type_val && !strcmp(type_val, "textarea"))
            ...
            html_line(cx, "<input%s%s%s />",
                      type_val ? lfmt(cx, " type=\"%s\"", type_val) : "", classattr, A);
        goto done;
    }
