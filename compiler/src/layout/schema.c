...
typedef struct {
    const char *canon, *lang;
} word_reg_t;

typedef struct {
    const char *group, *canon, *lang;
} value_reg_t;
...
static layout_elem_type_t kind_of(const char *k)
{
    ...
    if (!strcmp(k, "Single2Mother") || !strcmp(k, "single2mother"))
        return LE_SINGLE2MOTHER;
    if (!strcmp(k, "Mother2Single") || !strcmp(k, "mother2single"))
        return LE_MOTHER2SINGLE;
    return LE_MOTHER;
}
...
static void load_file(arena_t *a, logger_t *log, langpack_t *pack, const char *path,
                      layout_elem_def_t *defs, size_t *n)
{
    source_file_t *src = source_load(a, path);
    if (!src) return;
    token_stream_t *toks = NULL;
    lexer_run(a, log, pack, src, &toks);
    ast_node_t *prog = NULL;
    parser_run(a, log, toks, &prog);
    if (!prog) return;
    {
        size_t i = 0;
        for (; i < prog->list.len && *n < SCHEMA_MAX_ELEMS; i++) {
            ast_node_t *d = (ast_node_t *)prog->list.data[i];
            if (d->kind != AST_CONST_DECL || !d->a || d->a->kind != AST_STRUCT_LIT)
                continue;
...
static void load_attr_file(arena_t *a, logger_t *log, langpack_t *pack, const char *path,
                           layout_attr_def_t *defs, size_t *n)
{
    source_file_t *src = source_load(a, path);
    if (!src) return;
    token_stream_t *toks = NULL;
...
void layout_schema_init(const char *root)
{
    char base[480];
    if (root && root[0])
        sal_snprintf(base, sizeof base, "%s/std/layout", root);
    else
    ...
    if (nf > 0) {
        ...
        size_t n = 0;
        {
            int i = 0;
            for (; i < nf; i++)
                load_file(arena, log, pack, files[i], defs, &n);
        }
...

    {
        ...
        nf = list_schema_files(arena, dir, files, SCHEMA_MAX_ELEMS);
        {
            int i = 0;
            for (; i < nf; i++)
                load_attr_file(arena, log, pack, files[i], adefs, &an);
        }
    ...
    nf = list_schema_files(arena, dir, files, SCHEMA_MAX_ELEMS);
    {
        int i = 0;
        for (; i < nf; i++)
            load_value_file(arena, log, pack, files[i]);
    }
    logger_free(log);
}
