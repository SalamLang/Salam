...
static void lx_scan_one(lx_t *L)
{
    ...
    if (lx_at_digit(L, 0))
    ...
    else if (c == '.' && lx_at_digit(L, 1))
        lx_scan_number(L);
    else if (c == 'u' && (lx_peek2(L, 1) == '"' || lx_peek2(L, 1) == '\''))
        lx_scan_utf8_char(L);
    else if ((unsigned char)c >= 0x80 && lx_scan_unicode_op(L)) {
    } else if (lx_is_ident_start(c))
        lx_scan_ident(L);
    else if (c == '"') {
        if (lx_peek2(L, 1) == '"' && lx_peek2(L, 2) == '"')
            lx_scan_triple(L);
        else
            lx_scan_string(L);
    } else if (c == '\'')
        ...
        lx_scan_raw_string(L);
    else if (c == '@' && lx_is_ident_start(lx_peek2(L, 1)))
        lx_scan_meta(L);
    else
        lx_scan_op(L);
}
...
static void lx_emit_eof(lx_t *L)
{
    ...
    t.span.begin = LX_POS(L);
    t.span.end = t.span.begin;
    t.span.length = 0;
                  ...
                  const source_file_t *src, bool keep_comments, token_stream_t **out)
{
    ...
    LOG_D(log, PH_LEXER, "lexing %s (%zu bytes, lang=%s)", src->path, src->len,
          langpack_code(pack));
    for (;;) {
        if (L.layout_mode) {
            if (lx_end(&L)) break;
    ...
    if (L.layout_mode) LOG_W(log, PH_LEXER, "unclosed layout block at end of file");
    lx_newline_terminator(&L);
    lx_emit_eof(&L);
    LOG_D(log, PH_LEXER, "lexing complete: %zu tokens%s", token_stream_count(L.out),
          L.had_error ? i18n_tr(" (with errors)") : "");
    *out = L.out;
    return !L.had_error;
}
