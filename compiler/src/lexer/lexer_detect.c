...
static size_t det_match_ci(const char *text, size_t p, size_t le, const char *tag,
                           size_t n)
{
    size_t k = 0;
    if (p + n > le) return 0;
...
static size_t det_match_bytes(const char *text, size_t p, size_t le, const char *tag,
                              size_t n)
{
    size_t k = 0;
    if (p + n > le) return 0;
    for (; k < n; k++)
        if (text[p + k] != tag[k]) return 0;
    return n;
}

const char *langpack_marker_code(const char *text, size_t len)
{
    static const char en_tag1[] = "lang";
    static const char en_tag2[] = "language";
    static const char en_word[] = "english";
    static const char fa_tag[] = "\xd8\xb2\xd8\xa8\xd8\xa7\xd9\x86";
    ...
    while (i < len && line < 8) {
        ...
        if (ls + 1 < len && text[ls] == '/' && text[ls + 1] == '/') {
            size_t p = ls + 2;
            for (; p < le; p++) {
                int fa_tagged = 0, ar_tagged = 0;
                size_t tlen = det_match_bytes(text, p, le, fa_tag, 8);
                if (tlen) fa_tagged = 1;
                if (!tlen) {
                    tlen = det_match_bytes(text, p, le, ar_tag, 6);
                    if (tlen) ar_tagged = 1;
                }
                ...
                if (fa_tagged || ar_tagged) {
                    if (det_match_bytes(text, q, le, ar_val, 14)) return "ar";
...
static int det_count_keywords(arena_t *a, logger_t *quiet, const langpack_t *pack,
                              const source_file_t *src)
{
                                  ...
                                  const langpack_t *fallback)
{
    ...
    static const char *codes[] = {"en", "fa", "ar"};
    logger_t *quiet = logger_new(stderr, LOG_OFF, false);
    const langpack_t *best = fallback;
    int best_kw = 0;
    {
        size_t i = 0;
