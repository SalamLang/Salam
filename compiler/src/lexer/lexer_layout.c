...
static bool layout_try_end_inline(lx_t *L, const src_pos_t *b)
{
    ...
    if (!matched_kw && strcmp(end_kw, "end") != 0) {
        char c0 = lx_peek(L), c1 = lx_peek2(L, 1), c2 = lx_peek2(L, 2),
             c3 = lx_peek2(L, 3);
        if (c0 == 'e' && c1 == 'n' && c2 == 'd' &&
            (c3 == ' ' || c3 == '\t' || c3 == '\r' || c3 == '\n' || c3 == '\0')) {
            matched_kw = "end";
...
static void layout_report_bad_name(lx_t *L, const src_pos_t *b, const char *name,
                                   size_t namelen)
{
    size_t fw = 0;
    while (fw < namelen && name[fw] != ' ' && name[fw] != '\t')
        fw++;
    if (fw == 3 && (name[0] == 'E' || name[0] == 'e') &&
        (name[1] == 'N' || name[1] == 'n') && (name[2] == 'D' || name[2] == 'd'))
