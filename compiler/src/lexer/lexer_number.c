...
static size_t scan_digits(lx_t *L, char base, bool *bad)
{
    ...
    for (;;) {
        bool digit = base ? lx_is_base_digit(lx_peek(L), base) : lx_at_digit(L, 0) != 0;
        if (digit) {
            if (base)
                lx_adv(L);
            else
...
static void decode_number(const char *clean, bool is_float, token_value_t *v,
                          bool *overflow)
{
    *overflow = false;
    if (is_float) {
...
void lx_scan_number(lx_t *L)
{
    ...
    bool based = lx_peek(L) == '0' && (c1 == 'x' || c1 == 'X' || c1 == 'b' || c1 == 'B' ||
                                       c1 == 'o' || c1 == 'O');
    bool is_float = false;
    if (based)
    ...
    char stackbuf[64];
    char *clean =
        (tlen < sizeof stackbuf) ? stackbuf : (char *)arena_alloc(L->a, tlen + 1);
