...
void lx_scan_op(lx_t *L)
{
    ...
    token_kind_t k;
    switch (c) {
        case '(': L->group_depth++; k = TK_LPAREN; break;
        case ')': if (L->group_depth > 0) L->group_depth--; k = TK_RPAREN; break;
        ...
        case ';': k = TK_STMT_END; break;
        case '.':
            
            if (lx_peek(L) == '.' && lx_peek2(L, 1) == '.') { lx_adv(L); lx_adv(L); k = TK_ELLIPSIS; }
            else k = TK_DOT;
            break;
        case '+': k = lx_match(L, '=') ? TK_PLUS_EQ : TK_PLUS; break;
        case '-': k = lx_match(L, '=') ? TK_MINUS_EQ : TK_MINUS; break;
        case '*':
            if      (lx_match(L, '*')) k = TK_POWER;
            else if (lx_match(L, '=')) k = TK_STAR_EQ;
            else                       k = TK_STAR;
            break;
        case '/': k = lx_match(L, '=') ? TK_SLASH_EQ : TK_SLASH; break;
        case '%': k = lx_match(L, '=') ? TK_PERCENT_EQ : TK_PERCENT; break;
        case '=':
            if      (lx_match(L, '=')) k = TK_EQ;
            else if (lx_match(L, '>')) k = TK_FAT_ARROW;
            else                       k = TK_ASSIGN;
            break;
        case '!':
            if (lx_match(L, '=')) k = TK_NE;
            
            else if (lx_peek(L) == '_' && lx_peek2(L,1) == '_' && !lx_is_ident_cont(lx_peek2(L,2)))
                { lx_adv(L); lx_adv(L); k = TK_KW_PRINTERRLN; }
            else if (lx_peek(L) == '_' && !lx_is_ident_cont(lx_peek2(L,1)))
                { lx_adv(L); k = TK_KW_PRINTERR; }
            else k = TK_NOT;
            break;
        case '<': k = lx_match(L, '=') ? TK_LE : TK_LT; break;
        case '>': k = lx_match(L, '=') ? TK_GE : TK_GT; break;
        case '&': k = lx_match(L, '&') ? TK_AND : TK_AMP; break;
        case '|':
            if (lx_match(L, '|')) k = TK_OR;
            else { lx_error(L, &b, "unexpected '|' (use '||')"); return; }
            break;
        default:
    ...
    if (k == TK_COLON && (prev == TK_KW_LAYOUT ||
                          (L->comp_header && L->group_depth == 0))) {
        L->layout_mode = true;
        L->layout_depth = 1;
