...
{
    switch (k) {
        case TK_PLUS:    return isflt ? "fadd" : "add";
        case TK_MINUS:   return isflt ? "fsub" : "sub";
        case TK_STAR:    return isflt ? "fmul" : "mul";
        case TK_SLASH:   return isflt ? "fdiv" : (issigned ? "sdiv" : "udiv");
        case TK_PERCENT: return isflt ? "frem" : (issigned ? "srem" : "urem");
        default:         return NULL;
    }
}
...
{
    switch (k) {
        case TK_EQ: return isflt ? "oeq" : "eq";
        case TK_NE: return isflt ? "one" : "ne";
        case TK_LT: return isflt ? "olt" : (issigned ? "slt" : "ult");
        case TK_GT: return isflt ? "ogt" : (issigned ? "sgt" : "ugt");
        case TK_LE: return isflt ? "ole" : (issigned ? "sle" : "ule");
        case TK_GE: return isflt ? "oge" : (issigned ? "sge" : "uge");
        default:    return NULL;
    }
}
...
static const char *ll_str_operand(ll_t *ll, ast_node_t *n)
{
    ...
    if (ll_is_bool(ts)) {
        ...
        ll_emit(ll, "%s = call ptr @salam_ll_f64str(double %s)", r, ll_conv(ll, v, "f64"));
    } else if (ll_is_int(ts)) {
        if (ll_is_signed(ts)) ll_emit(ll, "%s = call ptr @salam_ll_i64str(i64 %s)", r, ll_conv(ll, v, "i64"));
        else                  ll_emit(ll, "%s = call ptr @salam_ll_u64str(i64 %s)", r, ll_conv(ll, v, "u64"));
    } else {
        ll_error(ll, n, "string concatenation with an unsupported operand type '%s'", ts ? ts : "?");
        return v.ref;
    }
    return r;
}
...
{
    switch (k) {
        case TK_PLUS: return "operator_add"; case TK_MINUS:   return "operator_sub";
        case TK_STAR: return "operator_mul"; case TK_SLASH:   return "operator_div";
        case TK_PERCENT: return "operator_mod"; case TK_POWER: return "operator_pow";
        case TK_EQ: return "operator_eq"; case TK_NE: return "operator_ne";
        case TK_LT: return "operator_lt"; case TK_GT: return "operator_gt";
        case TK_LE: return "operator_le"; case TK_GE: return "operator_ge";
        default: return NULL;
    }
}
                       ...
                       const char *mname, ast_node_t *rhs, llv_t *out)
{
    ...
    if (!sig) return false;
    const char *recvref = ll_is_ptr_ts(recv->type_str) ? ll_expr(ll, recv).ref
                                                       : ll_addr_of(ll, recv).ptr;
    sb_t ab; sb_init(&ab); sb_puts(&ab, ll_fmt(ll, "ptr %s", recvref));
    if (rhs) {
        const char *pts = sig->params.len
                        ? type_to_string(ll->sem->tc, (type_t *)sig->params.data[0]) : NULL;
...
bool ll_index_set(ll_t *ll, ast_node_t *idx, ast_node_t *value)
{
    ...
    ll_emit(ll, "call void @%s(ptr %s, %s %s, %s %s)", fname, recv,
            ll_ty(ll, p0), iv, ll_ty(ll, p1), vv);
    return true;
}
...
llv_t ll_binary(ll_t *ll, ast_node_t *n)
{
    ...
    if (ll_is_cmp(op)) {
        ...
        if (lp || rp) {
            ...
            const char *r = ll_new_tmp(ll);
            ll_emit(ll, "%s = icmp %s ptr %s, %s", r, ll_cmp_pred(op, false, false), lc, rc);
            return (llv_t){ r, "bool" };
        }
    ...
    if (!o) { ll_error(ll, n, "binary operator"); return ll_poison(rt); }
    const char *r = ll_new_tmp(ll);
...
static llv_t ll_unary(ll_t *ll, ast_node_t *n)
{
    ...
    if (n->op == TK_MINUS) {
        ...
        const char *r = ll_new_tmp(ll);
        if (ll_is_float(rt)) ll_emit(ll, "%s = fneg %s %s", r, ll_ty(ll, rt), cv);
        else                 ll_emit(ll, "%s = sub %s 0, %s", r, ll_ty(ll, rt), cv);
        return (llv_t){ r, rt };
    }
...
static void ll_lower_print(ll_t *ll, ast_node_t *n, bool nl, int err)
{
    ...
    { size_t i = 0; for (; i < segs.len; i++) {
        ...
        switch (s->kind) {
            case PF_STR:  sb_puts(&args, ll_fmt(ll, ", ptr %s", v.ref)); break;
            case PF_BOOL: {
                const char *t = ll_strconst(ll, "true"), *f = ll_strconst(ll, "false");
            ...
            case PF_CHAR: sb_puts(&args, ll_fmt(ll, ", i32 %s", ll_conv(ll, v, "i32"))); break;
            case PF_F64:  sb_puts(&args, ll_fmt(ll, ", double %s", ll_conv(ll, v, "f64"))); break;
            case PF_I32:  sb_puts(&args, ll_fmt(ll, ", i32 %s", ll_conv(ll, v, "i32"))); break;
            case PF_U32:  sb_puts(&args, ll_fmt(ll, ", i32 %s", ll_conv(ll, v, "u32"))); break;
            case PF_I64:  sb_puts(&args, ll_fmt(ll, ", i64 %s", v.ref)); break;
    ...
    else     ll_emit(ll, "%s = call i32 (ptr, ...) @printf(ptr %s%s)", t, f, sb_cstr(&args));
...
static void ll_emit_arg(ll_t *ll, sb_t *ab, ast_node_t *arg, func_sig_t *sig, size_t i)
{
    ...
    const char *pts = i < sig->params.len
                    ? type_to_string(ll->sem->tc, (type_t *)sig->params.data[i]) : v.ts;
...
static bool ll_call_str(ll_t *ll, ast_node_t *n, ast_node_t *obj, const char *m, llv_t *out)
{
    ...
    if (!strcmp(m, "substr") && na == 2) {
        ...
        r = ll_new_tmp(ll);
        ll_emit(ll, "%s = call ptr @salam_ll_substr(ptr %s, i32 %s, i32 %s)", r, recv, s, l);
        *out = (llv_t){ r, "str" }; return true;
    }
    if (!strcmp(m, "to_int")) {
        const char *x = ll_new_tmp(ll); r = ll_new_tmp(ll);
...
static llv_t ll_call_dyn(ll_t *ll, ast_node_t *n, ast_node_t *obj, const char *iface, const char *mname)
{
    ...
    const char *data = ll_new_tmp(ll), *vt = ll_new_tmp(ll), *sl = ll_new_tmp(ll), *fn = ll_new_tmp(ll);
...
static llv_t ll_call_method(ll_t *ll, ast_node_t *n, ast_node_t *callee)
{
    ...
    if (!strcmp(ots, "str")) { llv_t o; if (ll_call_str(ll, n, obj, mname, &o)) return o; }
    ...
    if (ms && ms->kind == SYM_METHOD && ms->overloads.len) {
        ...
        return ll_emit_call(ll, n, sig, ll_fmt(ll, "ptr %s", recv),
                            ll_mangle(ll, sname, mname, sig), type_to_string(ll->sem->tc, sig->ret));
    }
    ...
    if (impl && impl->kind == SYM_TYPEIMPL) {
        ...
        if (im && im->kind == SYM_METHOD && im->overloads.len) {
            ...
            return ll_emit_call(ll, n, sig, ll_fmt(ll, "%s %s", ll_ty(ll, ots), rv.ref),
                                ll_mangle_ti(ll, ots, mname, sig), type_to_string(ll->sem->tc, sig->ret));
        }
    }

    ll_error(ll, n, "method '%s' on type '%s' (or overloaded/builtin method)", mname, ots);
    return ll_poison(n->type_str);
}
...
static bool ll_call_intrinsic(ll_t *ll, ast_node_t *n, const char *nm, llv_t *out)
{
    ...
    if (!strcmp(nm, "hash") && na == 1) {
        ...
        if (ll_is_str(a0->type_str))
            ll_emit(ll, "%s = call i64 @salam_ll_strhash(ptr %s)", r, ll_expr(ll, a0).ref);
        else
            ll_emit(ll, "%s = call i64 @salam_ll_inthash(i64 %s)", r, ll_conv(ll, ll_expr(ll, a0), "u64"));
        *out = (llv_t){ r, "u64" }; return true;
    }
    if (!strcmp(nm, "char_from_code") && na == 1) {   
        r = ll_new_tmp(ll);
        ll_emit(ll, "%s = call ptr @salam_ll_charstr(i32 %s)", r, ll_conv(ll, ll_expr(ll, a0), "i32"));
        *out = (llv_t){ r, "str" }; return true;
    }
    return false;
}
...
static llv_t ll_call(ll_t *ll, ast_node_t *n)
{
    ...
    if (callee && callee->kind == AST_IDENTIFIER) {
        ...
        if (!strcmp(nm, "print")   || !strcmp(nm, "_"))  { ll_lower_print(ll, n, false, 0); return (llv_t){ "0", "void" }; }
        if (!strcmp(nm, "println") || !strcmp(nm, "__")) { ll_lower_print(ll, n, true,  0); return (llv_t){ "0", "void" }; }
        if (!strcmp(nm, "printerr"))                     { ll_lower_print(ll, n, false, 1); return (llv_t){ "0", "void" }; }
        if (!strcmp(nm, "printerrln"))                   { ll_lower_print(ll, n, true,  1); return (llv_t){ "0", "void" }; }
        ...
        if (!strcmp(nm, "sizeof") && n->list.len == 1) { 
            ...
            ll_emit(ll, "%s = getelementptr %s, ptr null, i32 1", szp, ll_ty(ll, t ? t : "i32"));
        ...
        if (!fsym) { llv_t o; if (ll_call_intrinsic(ll, n, nm, &o)) return o; }
...
ll_addr_t ll_addr_of(ll_t *ll, ast_node_t *n)
{
    switch (n->kind) {
        case AST_IDENTIFIER: {
            ...
            if (ll->cur_lambda) {
                ...
                { size_t i = 0; for (; i < caps->len; i++) {
                    ...
                    if (!strcmp(c->name, n->name)) {
                        const char *r = ll_new_tmp(ll);
                        ll_emit(ll, "%s = getelementptr %s, ptr %s, i32 0, i32 %zu",
            ...
            if (ll->self_ts) {
                ...
                if (idx >= 0) {
                    ...
                    const char *r = ll_new_tmp(ll);
                    ll_emit(ll, "%s = getelementptr inbounds %s, ptr %s, i32 0, i32 %d",
                            r, ll_struct_ltype(ll, ll->self_ts), ll->this_ref, idx);
        ...
        case AST_MEMBER: return ll_member_addr(ll, n);
        case AST_INDEX:  return ll_index_addr(ll, n);
        default: {
            llv_t v = ll_expr(ll, n);
...
static ll_addr_t ll_member_addr(ll_t *ll, ast_node_t *n)
{
    ...
    const char *r = ll_new_tmp(ll);
    ll_emit(ll, "%s = getelementptr inbounds %s, ptr %s, i32 0, i32 %d",
            r, ll_struct_ltype(ll, sname), base, idx);
    return (ll_addr_t){ r, fts };
}
...
static ll_addr_t ll_index_addr(ll_t *ll, ast_node_t *n)
{
    ...
    if (ll_is_ptr_ts(ots)) {
        ...
        const char *idx  = ll_conv(ll, ll_expr(ll, n->b), "i64");
        const char *r = ll_new_tmp(ll);
        ll_emit(ll, "%s = getelementptr inbounds %s, ptr %s, i64 %s", r, ll_ty(ll, ets), base, idx);
        return (ll_addr_t){ r, ets };
    }
    ll_addr_t b = ll_addr_of(ll, n->a);
    ...
    const char *idx = ll_conv(ll, ll_expr(ll, n->b), "i64");
    const char *r = ll_new_tmp(ll);
    ll_emit(ll, "%s = getelementptr inbounds %s, ptr %s, i64 0, i64 %s", r, ll_ty(ll, ots), b.ptr, idx);
    return (ll_addr_t){ r, ets };
}
...
static llv_t ll_struct_lit(ll_t *ll, ast_node_t *n)
{
    ...
    if (!ss) {
        ll_error(ll, n, "struct literal of unknown/unsupported type '%s'", n->name ? n->name : "?");
        return ll_poison(n->type_str);
    }
...
static llv_t ll_literal(ll_t *ll, ast_node_t *n)
{
    switch (n->op) {
        ...
        case TK_KW_TRUE:  return (llv_t){ "true",  "bool" };
        case TK_KW_FALSE: return (llv_t){ "false", "bool" };
        case TK_KW_NULL:  return (llv_t){ "null",  n->type_str ? n->type_str : "i32*" };
...
llv_t ll_expr(ll_t *ll, ast_node_t *n)
{
    ...
    switch (n->kind) {
        case AST_LITERAL: return ll_literal(ll, n);
        case AST_IDENTIFIER: {                 
            ll_addr_t a = ll_addr_of(ll, n);
            const char *r = ll_new_tmp(ll);
            ll_emit(ll, "%s = load %s, ptr %s", r, ll_ty(ll, a.ts), a.ptr);
        ...
        case AST_THIS: {
            if (!ll->this_ref) { ll_error(ll, n, "'this' outside a method"); return ll_poison(n->type_str); }
            if (ll->self_byval) {   
                const char *r = ll_new_tmp(ll);
        ...
        case AST_BINARY: return ll_binary(ll, n);
        case AST_UNARY:  return ll_unary(ll, n);
        case AST_CAST: {
            llv_t v = ll_expr(ll, n->a);
            const char *to = n->type && n->type->type_str ? n->type->type_str
