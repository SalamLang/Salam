...
const char *ll_func_ret(ll_t *ll, const char *ts)
{
    ...
    const char *p = ts + 4; int d = 0;          
    for (; *p; p++) { if (*p == '(') d++; else if (*p == ')') { if (--d == 0) { p++; break; } } }
    while (*p == ' ') p++;
...
const char *ll_conv(ll_t *ll, llv_t v, const char *to_ts)
{
    ...
    bool fi = ll_is_int(from), ff = ll_is_float(from), fp = ll_is_str(from) || ll_is_ptr_ts(from);
    bool ti = ll_is_int(to_ts), tf = ll_is_float(to_ts), tp = ll_is_str(to_ts) || ll_is_ptr_ts(to_ts);
    const char *lf = ll_ty(ll, from), *lt = ll_ty(ll, to_ts);
    const char *r = ll_new_tmp(ll);
    if (fi && ti) {
        int bf = ll_int_bits(from), bt = ll_int_bits(to_ts);
        if (bt == bf) return v.ref;   
        if (bt < bf) ll_emit(ll, "%s = trunc %s %s to %s", r, lf, v.ref, lt);
        else if (ll_is_signed(from)) ll_emit(ll, "%s = sext %s %s to %s", r, lf, v.ref, lt);
        else ll_emit(ll, "%s = zext %s %s to %s", r, lf, v.ref, lt);
        return r;
    }
    if (ff && tf) {
        if (!strcmp(from,"f32") && !strcmp(to_ts,"f64")) ll_emit(ll, "%s = fpext float %s to double", r, v.ref);
        else ll_emit(ll, "%s = fptrunc double %s to float", r, v.ref);
        return r;
    }
    if (fi && tf) {
        ll_emit(ll, "%s = %s %s %s to %s", r, ll_is_signed(from) ? "sitofp" : "uitofp", lf, v.ref, lt);
        return r;
    }
    if (ff && ti) {
        ll_emit(ll, "%s = %s %s %s to %s", r, ll_is_signed(to_ts) ? "fptosi" : "fptoui", lf, v.ref, lt);
        return r;
    }
    if (fi && tp) { ll_emit(ll, "%s = inttoptr %s %s to ptr", r, lf, v.ref); return r; }
    if (fp && ti) { ll_emit(ll, "%s = ptrtoint ptr %s to %s", r, v.ref, lt); return r; }
...
const char *ll_as_i1(ll_t *ll, llv_t v)
{
    ...
    const char *r = ll_new_tmp(ll);
    if (ll_is_float(v.ts)) ll_emit(ll, "%s = fcmp one %s %s, 0.0", r, ll_ty(ll, v.ts), v.ref);
    else                   ll_emit(ll, "%s = icmp ne %s %s, 0", r, ll_ty(ll, v.ts), v.ref);
    return r;
}
