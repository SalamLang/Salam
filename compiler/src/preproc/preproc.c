...
{
    if (s_nos < 0) { s_nos = 0; while (s_os_defs[s_nos]) s_nos++; }
    return s_nos;
}
    ...
    while (*p) {
        ...
        if (strncmp(t, "@ifdef", 6) == 0 && (t[6] == ' ' || t[6] == '\t')) {
            ...
            if (depth < PP_MAX_DEPTH) {
                stack[depth].active = c; stack[depth].taken = c; stack[depth].parent = parent;
                depth++;
            } else over++;
            ...
            if (depth > 0) {
                ...
                if (has_cond) {
                    ...
                    LOG_T(log, PH_DRIVER, "preproc: @else <cond> -> %s", f->active ? "active" : "skipped");
                } else {
                    f->active = f->parent && !f->taken;
                    if (f->active) f->taken = true;
                    LOG_T(log, PH_DRIVER, "preproc: @else -> %s", f->active ? "active" : "skipped");
                }
            }
