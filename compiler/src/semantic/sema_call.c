...
type_t *check_call(sema_t *s, ast_node_t *n)
{
    ...
    if (callee && callee->kind == AST_IDENTIFIER && strcmp(callee->name, "funcptr") == 0) {
        if (n->list.len != 1 || ((ast_node_t *)n->list.data[0])->kind != AST_IDENTIFIER) {
    ...
    if (callee && callee->kind == AST_IDENTIFIER && strcmp(callee->name, "callhandler") == 0) {
        if (n->list.len != 2) {
            SERR(s, 12, &n->span, "callhandler(fn, arg) takes a function pointer and an i64 argument");
    ...
    if (callee && callee->kind == AST_IDENTIFIER) {
        ...
        if (strcmp(nm,"print")==0||strcmp(nm,"_")==0||strcmp(nm,"println")==0||strcmp(nm,"__")==0
    ...
    if (callee && callee->kind == AST_MEMBER && callee->a && callee->a->kind == AST_IDENTIFIER) {
        ...
        if (pk && pk->kind == SYM_PACKAGE) {
            ...
            if (!fn || fn->kind != SYM_FUNC) {
                SERR(s, 12, &n->span, "package '%s' has no exported function '%s'", pk->name, fname);
                return decorate(s, n, err_ty(s));
            }
            if (!fn->is_pub) {
    ...
    if (callee && callee->kind == AST_MEMBER) {
        ...
        if (objt->kind == TY_STR) {
            ...
            { type_t *r = try_impl_call(s, n, callee, objt, &argtypes); if (r) return r; }
