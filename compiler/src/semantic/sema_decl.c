...
{
    const char *spec = (d->value.kind == TV_STRING && d->value.as.s) ? d->value.as.s : NULL;
    if (!spec || !spec[0]) { SERR(s, 57, &d->span, "link directive needs a library name or path"); return; }
    ...
    if (full != spec)
        SERR(s, 57, &d->span, "linked library not found: '%s' (resolved to '%s')", spec, full);
    else
...
void sema_collect(sema_t *s, ast_node_t *program)
{
    { size_t i = 0; for (; i < program->list.len; i++) {
        ...
        if (d->kind == AST_STRUCT_DEF) {
            ...
            sym->decl = d; sym->is_pub = d->is_pub;
    ...
    { size_t i = 0; for (; i < program->list.len; i++) {
        ...
        if (d->synthetic) continue;   
        switch (d->kind) {
            case AST_LINK: check_link(s, d); break;
            case AST_TYPE_ALIAS: {
                ...
                { size_t j = 0; for (; j < d->list.len; j++) {
                    ...
                    if (m->kind == AST_FIELD) {
                        ...
                        if (f->type && f->type->kind == TY_DYN)
                            SERR(s, 2, &m->span,
                                 "field '%s': a `dyn` value cannot yet be a struct field; "
                                 "make '%s' generic (`<T: %s>`) or store the data another way",
                                 m->name, d->name, f->type->name ? f->type->name : "Iface");
                        if (scope_define(s->a, sym->members, f))
                            SERR(s, 27, &m->span, "duplicate field '%s' in struct '%s'", m->name, d->name);
            ...
            case AST_VAR_DECL:
            case AST_CONST_DECL: {
                symbol_t *g = symbol_new(s->a, d->kind == AST_CONST_DECL ? SYM_CONST : SYM_VAR, d->name);
                g->type = d->type ? sema_resolve_type(s, d->type) : NULL;
                g->is_mut = d->is_mut;
                g->is_pub = d->is_pub;
                g->decl = d;
                if (d->kind == AST_CONST_DECL && d->a &&
                    d->a->kind == AST_LITERAL && d->a->value.kind == TV_INT) {
                    g->has_ival = true; g->ival = (long long)d->a->value.as.i;
                }
...
static void check_function(sema_t *s, ast_node_t *fn, symbol_t *owner, func_sig_t *sig)
{
    ...
    if (fn->a && fn->a->kind == AST_BLOCK && fn->a->list.len == 0)
        SERR(s, 61, &fn->span, "empty function '%s' (its body must contain at least one statement)", fn->name);
            ...
            { size_t j = 0; for (; j < d->list.len; j++) {
                ...
                if (m->kind == AST_FUNC_DEF) {
                    ...
                    if (f && stamp_empty_intrinsic(s, m->a, f->type)) {
                        ...
                        if (f && m->a->kind == AST_STRUCT_LIT && f->type && f->type->kind == TY_STRUCT)
                            s->expected = f->type;
