...
static void g_subst(arena_t *a, ast_node_t *n, vec_t *params, vec_t *args)
{
    ...
    g_subst(a, n->d, params, args);
    {
        size_t i = 0;
        for (; i < n->list.len; i++)
            g_subst(a, (ast_node_t *)n->list.data[i], params, args);
    }
    {
        size_t i = 0;
        for (; i < n->dims.len; i++)
            g_subst(a, (ast_node_t *)n->dims.data[i], params, args);
    }
    if (n->kind == AST_TYPE && n->name) {
        {
            ...
            for (; i < params->len; i++) {
                ...
                vec_init(&merged);
                {
                    size_t k = 0;
                    for (; k < cc->dims.len; k++)
                        vec_push(a, &merged, cc->dims.data[k]);
                }
                {
                    size_t k = 0;
                    for (; k < n->dims.len; k++)
                        vec_push(a, &merged, n->dims.data[k]);
                }
                n->dims = merged;
                break;
            }
...
static const char *g_instance_name(sema_t *s, const char *base, vec_t *argtypes)
{
    ...
    {
        size_t i = 0;
        for (; i < argtypes->len && o < sizeof buf - 130; i++) {
            char san[128];
            g_sanitize(type_to_string(s->tc, (type_t *)argtypes->data[i]), san,
                       sizeof san);
                            ...
                            const char *tparam, const src_span_t *span)
{
    ...
    if (concrete && concrete->kind == TY_STRUCT && concrete->decl)
        ...
        symbol_t *owner = scope_lookup(
            s->global, impl_owner_key(s->a, type_to_string(s->tc, concrete)));
        if (owner) members = owner->members;
    }
    if (!members) {
        SERR(s, 2, span,
             "'%s' does not satisfy '%s': no struct definition or `impl` block provides "
             "its methods",
    ...
    {
        size_t i = 0;
        for (; i < iface->members->symbols.len; i++) {
                ...
                for (; k < sm->overloads.len; k++) {
                    ...
                    bool pm = true;
                    {
                        ...
                        for (; p < want->params.len; p++)
                            if (!type_equiv((type_t *)have->params.data[p],
                                            (type_t *)want->params.data[p])) {
                                pm = false;
                                break;
                            }
                    }
                    if (pm && type_equiv(have->ret, want->ret)) {
                        sig_ok = true;
                        break;
                    }
                }
            ...
            if (!sig_ok) {
                SERR(s, 2, span,
                     "'%s' implements method '%s' with a signature that does not match "
                     "interface '%s'",
...
static void g_check_bounds(sema_t *s, ast_node_t *tmpl, vec_t *argtypes,
                           const src_span_t *span)
{
    {
        size_t i = 0;
        for (; i < tmpl->typarams.len && i < argtypes->len; i++) {
            const char *bound = i < tmpl->typaram_bounds.len
                                    ? (const char *)tmpl->typaram_bounds.data[i]
                                    : NULL;
            if (bound)
    ...
    if (!sig) return;
    {
        ...
        for (; i < call->list.len && i < sig->params.len && i < argtypes->len; i++) {
            ...
            if (pt && pt->kind == TY_DYN)
                call->list.data[i] = coerce_to_dyn(
                    s, pt, (ast_node_t *)call->list.data[i], (type_t *)argtypes->data[i]);
        }
    }
}
                               ...
                               const src_span_t *span)
{
    ...
    {
        size_t i = 0;
        for (; i < targ_nodes->len; i++)
            vec_push(s->a, &argtypes,
                     sema_resolve_type(s, (ast_node_t *)targ_nodes->data[i]));
    }
...
static type_t *g_unify(ast_node_t *pty, type_t *at, const char *T)
{
    ...
    if (strcmp(pty->name, T) == 0 && pty->dims.len == 0) {
        if (pty->is_pointer) return (at->kind == TY_PTR) ? at->pointee : NULL;
        return at;
    }
    ...
    if (strcmp(pn, "HashMap") == 0 && at->kind == TY_MAP) {
        ...
        if (!u && pty->list.len >= 2)
            u = g_unify((ast_node_t *)pty->list.data[1], at->elem, T);
        return u;
    }
    ...
    if (pty->list.len > 0 && at->kind == TY_STRUCT && at->decl) {
        ...
        if (isym->generic_base && strcmp(isym->generic_base, pty->name) == 0) {
            size_t k = pty->list.len < isym->generic_args.len ? pty->list.len
                                                              : isym->generic_args.len;
            {
                size_t i = 0;
                for (; i < k; i++) {
                       ...
                       type_t *expected)
{
    ...
    {
        size_t t = 0;
        for (; t < tmpl->typarams.len; t++) {
            const char *T = (const char *)tmpl->typarams.data[t];
            type_t *bound = NULL;
            {
                size_t pi = 0;
                for (; pi < tmpl->list.len && pi < argtypes->len; pi++) {
