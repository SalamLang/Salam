...
static void ensure_schema(void)
{
    ...
    if (!ready) { layout_schema_init(NULL); ready = true; }
}
                      ...
                      const char **out_entry, char **out_diag)
{
    ...
    if (!pack) { *out_diag = strdup(i18n_tr("unknown language pack")); return false; }
...
const char *salam_web_run_app(const char *source, const char *lang)
{
    ...
    ast_node_t *program; sema_result_t *sema; const char *entry; char *diag = NULL;
    if (!front_end(arena, log, diagf, lang, source, &program, &sema, &entry, &diag)) {
        const char *r = set_result(diag); free(diag);
        logger_free(log); if (diagf) fclose(diagf); arena_free(arena);
        return r;
    }
    FILE *outf = tmpfile();
    ...
    sb_t b; sb_init(&b);
    sb_puts(&b, out);
    if (errs && errs[0]) { if (out && out[0] && out[strlen(out)-1] != '\n') sb_putc(&b, '\n'); sb_puts(&b, errs); }
    const char *r = set_result(sb_cstr(&b));
    sb_free(&b); free(out); free(errs);
    logger_free(log);
    if (outf)  fclose(outf);
...
const char *salam_web_build_layout(const char *source, const char *lang)
{
    ...
    if (!lb) {
        const char *r = set_result(i18n_tr("this program has no layout block; "
    ...
    const char *r;
    if (diag_e->errors) { char *d = slurp(diagf); r = set_result((d && d[0]) ? d : i18n_tr("layout build failed")); free(d); }
    else                  r = set_result(html);
    logger_free(log); if (diagf) fclose(diagf); arena_free(arena);
    return r;
}
...
static void ser_tokens(xml_writer_t *w, const void *o)  { tokens_to_xml(w, (const token_stream_t *)o); }

static void ser_ast(xml_writer_t *w, const void *o)     { ast_to_xml(w, (const ast_node_t *)o); }

static void ser_symbols(xml_writer_t *w, const void *o) { symbols_to_xml(w, (const sema_result_t *)o); }
...
const char *salam_web_emit(const char *source, const char *lang, const char *phase)
{
    ...
    const char    *r       = NULL;
    langpack_t *pack = langpack_load(lang);
    if (!pack) { r = set_result(i18n_tr("unknown language pack")); goto done; }
    ...
    if (!strcmp(phase, "c")) {
        codegen_output_t *cg = codegen_run(arena, log, program, sema, "main",
                                           false, false, "main.salam", entry);
        sb_t b; sb_init(&b);
        if (cg && cg->h_src) { sb_puts(&b, "/* ===== salam_mod_main.h ===== */\n"); sb_puts(&b, cg->h_src); sb_putc(&b, '\n'); }
        if (cg && cg->c_src) { sb_puts(&b, "/* ===== salam_mod_main.c ===== */\n"); sb_puts(&b, cg->c_src); }
        r = set_result(sb_cstr(&b)); sb_free(&b);
        goto done;
    }
    if (!strcmp(phase, "llvm")) {
        ...
        if (ll && ll->ll_src && ll->ll_src[0]) sb_puts(&b, ll->ll_src);
        else if (!ll || ll->ok)               sb_puts(&b, "; (no IR produced)\n");
        r = set_result(sb_cstr(&b)); sb_free(&b);
        goto done;
    }
