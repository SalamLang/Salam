...
rem Quick build of salam.exe (Windows cmd.exe equivalent of tools/bash/build-compiler.sh).
rem Keep the source file list in sync with build-compiler.sh whenever files are added there.
...
set "LDLIBS=-lm"
echo %CC%| findstr /i "tcc" >nul
if not errorlevel 1 set "LDLIBS="
...
if errorlevel 1 (
    echo Build failed with %CC%.
    exit /b 1
)

echo built .\salam.exe with %CC%
