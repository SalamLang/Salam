(function () {
  ...
  var I18N = {
    en: { title: "Salam Playground", mode_app: "App", mode_layout: "Layout",
          run: "Run", autorun: "Auto-run", editor: "Editor", output: "Output", preview: "Preview",
          ready: "ready", running: "running…", done: "done", error: "error", loading: "loading compiler…",
          custom: "Custom", examples: "Examples",
          g_console: "Console programs", g_web: "Web pages", badge_web: "WEB",
          theme_auto: "Auto", theme_light: "Light", theme_dark: "Dark",
          view_run: "Run", view_tokens: "Tokens", view_ast: "AST", view_symbols: "Symbols",
          view_html: "HTML", view_css: "CSS", view_js: "JS",
          view_c: "C", view_llvm: "LLVM IR", copy: "Copy", copied: "Copied" },
    fa: { title: "زمین بازی سلام", mode_app: "برنامه", mode_layout: "چیدمان",
          run: "اجرا", autorun: "اجرای خودکار", editor: "ویرایشگر", output: "خروجی", preview: "پیش‌نمایش",
          ready: "آماده", running: "در حال اجرا…", done: "انجام شد", error: "خطا", loading: "بارگذاری کامپایلر…",
          custom: "سفارشی", examples: "نمونه‌ها",
          g_console: "برنامه‌های کنسولی", g_web: "صفحه‌های وب", badge_web: "وب",
          theme_auto: "خودکار", theme_light: "روشن", theme_dark: "تاریک",
          view_run: "اجرا", view_tokens: "توکن‌ها", view_ast: "درخت نحو", view_symbols: "نمادها",
          view_html: "HTML", view_css: "CSS", view_js: "JS",
          view_c: "کد C", view_llvm: "LLVM IR", copy: "کپی", copied: "کپی شد" }
  };
  ...
  function highlight(src) {
    ...
    while ((m = TOK.exec(src)) !== null) {
      ...
      else if (m[4]) cls = (KW.has(tok) || EL.has(tok)) ? "k" : (TY.has(tok) ? "t" : null);
      else if (m[5]) cls = "o";
  ...
  function highlightAttrs(s) {
    return s.replace(/([\w:.\-]+)(\s*=\s*)("[^"]*"|'[^']*')/g, function (_, name, eq, val) {
      return '<span class="xa">' + esc(name) + "</span>" + esc(eq) +
             '<span class="xv">' + esc(val) + "</span>";
    });
  }
  var XML_TOK = /<!--[\s\S]*?-->|<\?[\s\S]*?\?>|<\/?[\w:.\-]+(?:"[^"]*"|'[^']*'|[^>])*>/g;
  function highlightXML(src) {
    ...
    while ((m = XML_TOK.exec(src)) !== null) {
      ...
      if (t.slice(0, 4) === "<!--") {
        ...
        var tm = /^(<\/?)([\w:.\-]+)([\s\S]*?)(\/?>)$/.exec(t);
        if (tm) {
          out += '<span class="xb">' + esc(tm[1]) + "</span>" +
                 '<span class="xt">' + esc(tm[2]) + "</span>" +
                 highlightAttrs(tm[3]) +
                 '<span class="xb">' + esc(tm[4]) + "</span>";
        } else {
          out += esc(t);
        }
      }
  ...
  var IR_KW = new Set([
    "define","declare","ret","br","switch","indirectbr","invoke","resume",
    "unreachable","call","tail","musttail","notail","callbr",
    "load","store","alloca","getelementptr","inbounds","fence","cmpxchg","atomicrmw",
    ...
    "and","or","xor","shl","lshr","ashr","neg","fneg",
    "icmp","fcmp","eq","ne","ugt","uge","ult","ule","sgt","sge","slt","sle",
    "oeq","ogt","oge","olt","ole","one","ord","uno","ueq","une",
    "trunc","zext","sext","fptrunc","fpext","fptoui","fptosi","uitofp","sitofp",
    "ptrtoint","inttoptr","bitcast","addrspacecast","select","phi","freeze","va_arg",
    "global","constant","private","internal","external","common","linkonce","weak",
    "appending","available_externally","extern_weak","dllimport","dllexport",
    "unnamed_addr","local_unnamed_addr","dso_local","dso_preemptable","thread_local",
    "align","nsw","nuw","exact","volatile","atomic","syncscope","seq_cst","acquire",
    "release","acq_rel","monotonic",
    "nounwind","noinline","alwaysinline","readonly","readnone","willreturn","nofree",
    "noalias","nocapture","dereferenceable","byval","sret","inreg","signext","zeroext",
    "noundef","nonnull","nest","returned","speculatable","mustprogress","norecurse",
    "target","datalayout","triple","source_filename","attributes","module","asm",
    "to","from","x","null","none","undef","poison","zeroinitializer","blockaddress",
    "true","false","type","opaque","label","metadata","distinct","section","comdat"
  ]);
  function isIRType(w) {
    return /^(i\d+|void|float|double|half|bfloat|fp128|x86_fp80|x86_mmx|ppc_fp128|ptr|token)$/.test(w);
  }
  ...
  function highlightIR(src) {
    ...
    while ((m = IR_TOK.exec(src)) !== null) {
      ...
      if (m[1]) cls = "ic";
        ...
        if (src.charCodeAt(IR_TOK.lastIndex) === 58 && src.charCodeAt(IR_TOK.lastIndex + 1) !== 58) {
          tok += ":"; IR_TOK.lastIndex++; cls = "ilbl";
        } else {
  ...
  var C_KW = new Set([
    ...
    "_Bool","_Complex","_Imaginary","_Atomic","_Noreturn","_Static_assert",
    "_Thread_local","_Alignas","_Alignof","_Generic",
    "NULL","true","false","nullptr"
  ]);
  var C_TY = new Set([
    "int8_t","int16_t","int32_t","int64_t","uint8_t","uint16_t","uint32_t",
    "uint64_t","intptr_t","uintptr_t","intmax_t","uintmax_t","ptrdiff_t",
    "size_t","ssize_t","wchar_t","char16_t","char32_t","bool","FILE","va_list",
  ...
  function highlightC(src) {
    ...
    while ((m = C_TOK.exec(src)) !== null) {
      ...
      var tok = m[0], cls = null;
      if (m[1]) cls = "cm";
      else if (m[2]) { out += highlightPre(tok); last = C_TOK.lastIndex; continue; }
      else if (m[3]) cls = "cs";
      else if (m[4]) cls = "cl";
      else if (m[5]) cls = "cn";
      else if (m[6]) {
  ...
  var CSS_TOK = /(\/\*[\s\S]*?\*\/)|("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')|(#[0-9A-Fa-f]{3,8}\b)|(-?(?:\d*\.\d+|\d+)(?:px|em|rem|%|vh|vw|vmin|vmax|pt|pc|cm|mm|in|ex|ch|deg|rad|turn|s|ms|fr|dpi)?)|(@[A-Za-z-]+)|(!important)|([A-Za-z_][\w-]*)|([{}();:,>~+*.#\[\]=])/g;
  ...
  var JS_KW = new Set([
    ...
    "await","of","get","set","null","true","false","undefined","NaN","Infinity"
  ]);
  var JS_TOK = /(\/\/[^\n]*|\/\*[\s\S]*?\*\/)|("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|`(?:[^`\\]|\\.)*`)|(\.?\d[\w.]*)|([A-Za-z_$][\w$]*)|([-+*\/%=!<>&|^~?:.,;(){}\[\]])/g;
  ...
  var editor = null;
  var lang = "en", mode = "app", autorun = true, themePref = "auto";
  var view = "run"; // run | tokens | ast | symbols | html | css | js | c | llvm
  var exampleId = "hello";
  var runTimer = null, saveTimer = null;
  var wasm = { ready: false, runApp: null, buildLayout: null, emit: null };
  var $ = function (id) { return document.getElementById(id); };
  function findExample(id) { for (var i = 0; i < EXAMPLES.length; i++) if (EXAMPLES[i].id === id) return EXAMPLES[i]; return null; }
  function renderHL() {
    var hl = $("hl"), ta = $("code");
    ...
    saveTimer = setTimeout(function () {
      try {
        localStorage.setItem(STORE_KEY, JSON.stringify({
          code: editor ? editor.getValue() : "", lang: lang, mode: mode,
          autorun: autorun, theme: themePref, example: exampleId, view: view
        }));
      } catch (e) { }
    }, 200);
  }
  ...
  function applyTheme() {
    ...
    b.querySelector(".theme-lbl").textContent = I18N[lang]["theme_" + themePref];
    if (editor) renderHL();
  }
  function cycleTheme() {
  ...
  var ICON = {
    ...
    layout: '<svg class="ic web" viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="1.5" y="2.7" width="13" height="10.6" rx="1.6"/><path d="M1.5 5.7h13"/><circle cx="3.7" cy="4.2" r=".55" fill="currentColor" stroke="none"/><circle cx="5.5" cy="4.2" r=".55" fill="currentColor" stroke="none"/></svg>',
    custom: '<svg class="ic" viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M10.5 2.6l2.9 2.9-8 8H2.6v-2.9z" stroke-linejoin="round"/></svg>'
  };
  function modeIcon(m) { return ICON[m] || ICON.app; }
  ...
  function rebuildDropdown() {
    var list = $("dd-list");
    list.innerHTML = "";
    [["app", I18N[lang].g_console], ["layout", I18N[lang].g_web]].forEach(function (g) {
      var items = EXAMPLES.filter(function (e) { return e.mode === g[0]; });
      ...
      items.forEach(function (ex) {
        ...
        li.dataset.id = ex.id;
        li.innerHTML = modeIcon(ex.mode) + '<span class="dd-text">' + esc(ex.title[lang]) + "</span>" +
                       (ex.mode === "layout" ? '<span class="dd-badge">' + esc(I18N[lang].badge_web) + "</span>" : "");
    ...
    if (exampleId === "custom") {
      var li = document.createElement("li");
      li.className = "dd-item sel custom"; li.dataset.id = "custom";
      li.innerHTML = modeIcon("custom") + '<span class="dd-text">' + esc(I18N[lang].custom) + "</span>";
      list.appendChild(li);
    }
    var curIcon = exampleId === "custom" ? modeIcon("custom") : modeIcon(mode);
    $("dd-label").innerHTML = curIcon + '<span class="dd-text">' + esc(dropdownLabel()) + "</span>";
  }
  function openDropdown(open) { $("dd-list").hidden = !open; $("examples").classList.toggle("open", open); }
  function defaultExampleFor(m) {
    for (var i = 0; i < EXAMPLES.length; i++) if (EXAMPLES[i].mode === m) return EXAMPLES[i].id;
  ...
  function scheduleRun() {
    ...
    runTimer = setTimeout(function () { if (autorun && wasm.ready) run(); }, 500);
  }
  function showPhase(src) {
    var pre = $("output");
    var txt = (wasm.emit ? wasm.emit(src, lang, view) : "(inspector unavailable)") || "(empty)";
    var isErr = /error\[|runtime error|خطا|compilation failed|layout build failed|no layout block/i.test(txt);
    var isXml = !isErr && (view === "tokens" || view === "ast" || view === "symbols" || view === "html");
    ...
    pre.className = "output codeview" + (isXml ? " xml" : "") + (isIR ? " ir" : "") +
  ...
  function run() {
    ...
    setTimeout(function () {
      try {
        if (view !== "run") { showPhase(src); return; }
        if (mode === "layout") {
          ...
          if (/^\s*</.test(res)) {
            ...
            $("preview").srcdoc =
              ...
              '<div dir="' + dir + '" style="font-family:Vazirmatn,system-ui,sans-serif;' +
  ...
  function showBanner(html) {
    ...
    b.className = "banner"; b.innerHTML = html;
  ...
  function bootWasm() {
    ...
    window.Module = {
      ...
      printErr: function (t) { console.warn("[salam]", t); },
      onRuntimeInitialized: function () {
        wasm.runApp = Module.cwrap("salam_web_run_app", "string", ["string", "string"]);
        wasm.buildLayout = Module.cwrap("salam_web_build_layout", "string", ["string", "string"]);
        wasm.emit = Module.cwrap("salam_web_emit", "string", ["string", "string", "string"]);
        wasm.ready = true;
        $("run").disabled = false;
        setStatus("ready", "ok");
        if (autorun) run();
    ...
    s.onerror = function () {
      setStatus("error", "err");
      showBanner('The WebAssembly build was not found. Build it with ' +
  ...
  function onEdit() {
    ...
    if (exampleId !== "custom") {
      var ex = findExample(exampleId);
      if (!ex || editor.getValue() !== ex.code[lang]) { exampleId = "custom"; rebuildDropdown(); }
    }
    saveState();
    scheduleRun();
  }
  function setupEditor() {
    ...
    editor = {
      ...
      setValue: function (v) { ta.value = v; ta.scrollTop = 0; renderHL(); }
    };
    ta.addEventListener("input", onEdit);
    ta.addEventListener("scroll", function () { var hl = $("hl"); hl.scrollTop = ta.scrollTop; hl.scrollLeft = ta.scrollLeft; });
    var INDENT = "    ";
    function setRange(start, end, text, selS, selE) {
      ...
      ta.selectionStart = selS; ta.selectionEnd = selE;
    }
    function blockIndent(outdent) {
      var val = ta.value, s = ta.selectionStart, e = ta.selectionEnd;
      var ls = val.lastIndexOf("\n", s - 1) + 1;
      var eAdj = (e > s && val.charAt(e - 1) === "\n") ? e - 1 : e;
      var le = val.indexOf("\n", eAdj); if (le < 0) le = val.length;
      var firstDelta = 0, totalDelta = 0;
      var out = val.slice(ls, le).split("\n").map(function (line, i) {
        var d = 0;
        if (outdent) {
          var m = line.match(/^( {1,4}|\t)/);
          if (m) { d = -m[0].length; line = line.slice(m[0].length); }
        } else if (line.length) {
          d = INDENT.length; line = INDENT + line;
        }
        if (i === 0) firstDelta = d;
        totalDelta += d;
        return line;
    ...
    ta.addEventListener("keydown", function (e) {
      ...
      if (e.key === "Tab") {
        ...
        e.preventDefault(); blockIndent(false);
      } else if (ctrl && (e.key === "[" || e.code === "BracketLeft")) {
        e.preventDefault(); blockIndent(true);
      } else if (e.key === "Enter" && ctrl) {
        e.preventDefault(); run();
      } else if (e.key === "Enter" && !e.shiftKey && !e.altKey) {
        e.preventDefault(); autoIndentEnter();
      }
    });
    function val_has_nl(v, s, e) { return v.slice(s, e).indexOf("\n") >= 0; }
    var saved = loadState() || {};
    var params = new URLSearchParams(location.search);
    themePref = params.get("theme") || saved.theme || "auto";
    autorun = params.get("autorun") === "0" ? false : (params.has("autorun") ? true : (saved.autorun !== false));
    lang = params.get("lang") || saved.lang || "en";
    exampleId = params.get("example") || saved.example || "hello";
    var VIEWS = ["run", "tokens", "ast", "symbols", "html", "css", "js", "c", "llvm"];
    view = params.get("view") || saved.view || "run";
    if (VIEWS.indexOf(view) < 0) view = "run";
    var ex = findExample(exampleId);
    if (ex) { mode = ex.mode; }
    else    { exampleId = "custom"; mode = params.get("mode") || saved.mode || "app"; }
    applyTheme();
    applyMode(mode);
    setAutorun(autorun);
  ...
  document.addEventListener("click", function (e) {
    var t = e.target;
    if (t.closest && t.closest("#menu-toggle")) { setMenu(!menuOpen()); return; }
    if (t.closest && t.closest("#scrim")) { setMenu(false); return; }
    var cpc = t.closest && t.closest("#copy-code");
    if (cpc) { doCopy(editor ? editor.getValue() : "", cpc); return; }
    var cpo = t.closest && t.closest("#copy-out");
    if (cpo) { doCopy(resultText(), cpo); return; }
    var seg = t.closest && t.closest(".seg-btn, #autorun, #run, #theme");
    if (seg) {
      if (seg.id === "run") { if (view !== "run") { view = "run"; applyView(); saveState(); } run(); }
      ...
      else if (seg.dataset.mode) { if (seg.dataset.mode !== mode) selectExample(defaultExampleFor(seg.dataset.mode)); }
      if (!seg.dataset.view) setMenu(false);
      return;
    }
    var ddBtn = t.closest && t.closest("#dd-button");
    if (ddBtn) { openDropdown($("dd-list").hidden); return; }
    var item = t.closest && t.closest(".dd-item");
    if (item) { openDropdown(false); selectExample(item.dataset.id); setMenu(false); return; }
    openDropdown(false);
  });
  ...
  setInterval(function () { if (themePref === "auto") applyTheme(); }, 60000);
  function setupSplit() {
    var split = document.querySelector(".split"), gutter = $("gutter");
    if (!split || !gutter) return;
    var SPLIT_KEY = "salam.split", MIN = 140, GUT = 6;
    var rowsMQ = window.matchMedia("(max-width: 900px)");
    var isRows = function () { return rowsMQ.matches; };
    var saved2;
    try { saved2 = JSON.parse(localStorage.getItem(SPLIT_KEY) || "null"); } catch (e) { saved2 = null; }
    function applySaved() {
      if (!saved2) return;
      if (isRows()) { if (saved2.rows != null) split.style.setProperty("--row-a", saved2.rows + "px"); }
      else          { if (saved2.cols != null) split.style.setProperty("--col-a", saved2.cols + "px"); }
    }
    applySaved();
    var dragging = false;
    function onMove(e) {
      ...
      if (isRows()) {
        var h = Math.max(MIN, Math.min(e.clientY - rect.top, rect.height - MIN - GUT));
    ...
    function onUp() {
      ...
      if (!saved2) saved2 = {};
      if (isRows()) saved2.rows = parseFloat(split.style.getPropertyValue("--row-a"));
      else          saved2.cols = parseFloat(split.style.getPropertyValue("--col-a"));
      try { localStorage.setItem(SPLIT_KEY, JSON.stringify(saved2)); } catch (e) { }
    }
    ...
    (rowsMQ.addEventListener ? rowsMQ.addEventListener.bind(rowsMQ, "change")
