...
async function cmdBuild() {
  ...
  if (!doc) return;
  const file = doc.uri.fsPath;
  const out = file.replace(/\.salam$/i, process.platform === 'win32' ? '.exe' : '');
  runInTerminal(['build', file, '--output=' + out, ...commonArgs(doc)], path.dirname(file));
}

function execCapture(args, cwd) {
  return new Promise((resolve) => {
    cp.execFile(compilerPath(), args, { cwd, maxBuffer: 32 * 1024 * 1024 }, (err, stdout, stderr) => {
      resolve({
  ...
  vscode.window
    ...
    .then((pick) => {
      if (pick === 'Open Settings') {
        vscode.commands.executeCommand('workbench.action.openSettings', 'salam.compilerPath');
      }
    });
}
...
async function cmdEmitXml(flag, languageTitle) {
  ...
  if (!doc) return;
  const file = doc.uri.fsPath;
  const res = await execCapture([file, flag, ...commonArgs(doc)], path.dirname(file));
  if (res.spawnError) return reportCompilerMissing();
  const body = res.stdout.trim() || res.stderr.trim() || '(no output)';
  const shown = await vscode.workspace.openTextDocument({ content: body, language: 'xml' });
  await vscode.window.showTextDocument(shown, { preview: true });
  if (res.code !== 0 && res.stderr.trim()) {
    output.appendLine(`[${languageTitle}] exited ${res.code}: ${res.stderr.trim()}`);
  }
}
...
async function formatText(text, lang) {
  ...
  try {
    ...
    if (res.code !== 0) {
      ...
      vscode.window.showErrorMessage('Salam: formatting failed (see "Salam" output for details).');
      return null;
    }
...
const formattingProvider = {
  async provideDocumentFormattingEdits(document) {
    ...
    const formatted = await formatText(document.getText(), detectLang(document));
...
async function cmdNewProject() {
  const name = await vscode.window.showInputBox({
    ...
    validateInput: (v) => (/^[\w.-]+$/.test(v) ? null : 'Use letters, digits, dots, hyphens or underscores.'),
  });
  if (!name) return;
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('salam.run', () => cmdRunWith(cfg().get('run.command', 'exec'))),
    ...
    vscode.commands.registerCommand('salam.emitTokens', () => cmdEmitXml('--emit-tokens-xml', 'tokens')),
    vscode.commands.registerCommand('salam.emitAst', () => cmdEmitXml('--emit-ast-xml', 'ast')),
