...
const CONTROL = ['if','else','while','for','ret','break','continue','repeat','to','every','defer','end',
  'اگر','وگرنه','تاوقتی','برای','بازگشت','بشکن','ادامه','تکرار','تا','هر','تعلیق','پایان'];

const STORAGE = ['func','mut','const','type','struct','enum','interface','impl','package','import',
  'as','pub','component','layout','operator','extern',
  'تابع','متغیر','ثابت','نوع','ساختار','شمارش','واسط','پیاده‌سازی','بسته','واردکردن','بعنوان','عمومی','مولفه','صفحه','عملگر','خارجی'];

const IO = ['printerrln','printerr','println','print','input',
  'خطاچاپ','خطابنویس','چاپ','بنویس','بخوان'];

const CONSTS = ['true','false','null','درست','نادرست','پوچ'];
const THIS = ['this','این'];
const AUTO = ['auto','خودکار'];
const CONTEXTUAL = ['link','static','dynamic','framework','kind','dyn',
  'پیوند','ایستا','پویا','چارچوب','گونه'];

const TYPES = ['i8','i16','i32','i64','u8','u16','u32','u64','f32','f64','isize','usize','bool','str','char','void',
  'صحیح۸','صحیح۱۶','صحیح۳۲','صحیح۶۴','اعشاری۳۲','اعشاری۶۴','رشته','منطقی','بایت','کاراکتر'];

const FUNC_DEF_KW = ['func','تابع'];
const TYPE_DEF_KW = ['type','struct','enum','interface','component','package',
  'نوع','ساختار','شمارش','واسط','مولفه','بسته'];
  ...
  repository: {
    directives: {
      patterns: [{
        ...
        patterns: [
          { match: '\\b(mode|title|lang|dir|name|version)\\b', name: 'keyword.other.directive.salam' },
    ...
    comments: {
      patterns: [
        { name: 'comment.block.salam', begin: '/\\*', beginCaptures: { 0: { name: 'punctuation.definition.comment.salam' } }, end: '\\*/', endCaptures: { 0: { name: 'punctuation.definition.comment.salam' } } },
        { name: 'comment.line.double-slash.salam', begin: '//', beginCaptures: { 0: { name: 'punctuation.definition.comment.salam' } }, end: '$' },
      ],
    },
    'meta-annotations': {
      patterns: [{
    ...
    'string-escapes': {
      patterns: [
        { name: 'constant.character.escape.salam', match: '\\\\(?:x[0-9A-Fa-f]{2}|u\\{[0-9A-Fa-f]+\\}|u[0-9A-Fa-f]{4}|[0abfnrtv"\'\\\\])' },
        { name: 'invalid.illegal.unknown-escape.salam', match: '\\\\.' },
      ],
    },
    numbers: {
      patterns: [
        { name: 'constant.numeric.float.salam', match: `${NBB}(?:[0-9][0-9_]*\\.[0-9][0-9_]*(?:[eE][+-]?[0-9][0-9_]*)?|[0-9][0-9_]*[eE][+-]?[0-9][0-9_]*)f?${NBA}` },
        ...
        { name: 'constant.numeric.integer.salam', match: `${NBB}[0-9][0-9_]*f?${NBA}` },
      ],
    },
    definitions: {
      patterns: [
