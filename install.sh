...
case "$os" in
  ...
  FreeBSD|OpenBSD|NetBSD|DragonFly)
    die "Salam does not publish native $os binaries yet. Linux and macOS binaries are ELF/Mach-O and will not run here. Follow BUILD instructions in the repository README to compile from source: https://github.com/${REPO}"
    ;;
  MINGW*|MSYS*|CYGWIN*)
...
if [ -n "$VERSION" ]; then
  ...
  for tag in $tags; do
    ...
    if fetch_to_file "$url" "$archive" 2>/dev/null && [ -s "$archive" ]; then
      VERSION="$v"; ASSET="$asset"; URL="$url"; found=1
      break
    fi
    rm -f "$archive"
  done
