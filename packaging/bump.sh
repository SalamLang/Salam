...
[ -n "$VERSION" ] || { echo "usage: packaging/bump.sh <version> [seed-version]" >&2; exit 2; }
VERSION=${VERSION#v}

need() { command -v "$1" >/dev/null 2>&1 || { echo "bump.sh: $1 is required" >&2; exit 1; }; }
need curl
need python3
...
sum_for() {
    ...
    [ -n "$v" ] || { echo "bump.sh: $name is not listed in SHA256SUMS" >&2; exit 1; }
    printf '%s' "$v"
}
sum_opt() {
...
print(best or "")
' "$VERSION")
    [ -n "$SEED_VERSION" ] || { echo "bump.sh: could not find a seed release older than $VERSION" >&2; exit 1; }
fi
...
for pair in \
    ...
    "linux-aarch64:$SHA256_SEED_LINUX_AARCH64"
do
    [ -n "${pair#*:}" ] || {
...
render "$HERE/aur/salam/PKGBUILD"                       "$DIST/aur/salam/PKGBUILD"
