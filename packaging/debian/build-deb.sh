...
while [ $# -gt 0 ]; do
    ...
    --compression-level=*) COMPRESSION_LEVEL=${1#*=}; shift ;;
...
do
    ...
    [ -f "$tarball" ] || { echo "skip $deb_arch: no $(basename "$tarball")"; continue; }
    ...
    [ -n "$src" ] || { echo "  $(basename "$tarball") unpacked to nothing" >&2; exit 1; }
...
[ "$built" -gt 0 ] || { echo "build-deb.sh: no tarballs matched in $INPUT" >&2; exit 1; }
