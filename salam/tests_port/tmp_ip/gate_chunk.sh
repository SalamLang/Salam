#!/bin/sh
# Runs one chunk (list of basenames, one per line) of en/general/*.salam
# through both binaries and appends results to the shared bonus logs.
set -u
cd /mnt/c/Users/Admin/Projects/SalamLang/Salam
export SALAM_STD="$PWD/compiler"
REF=/tmp/salam
PORT=/tmp/ipport
CHUNK="$1"
GEN_MISMATCH_LOG=/tmp/ip_gate_general_mismatches.txt
COUNT_FILE=/tmp/ip_gate_bonus_count.txt

gident=0
gtotal=0
while IFS= read -r name; do
    [ -z "$name" ] && continue
    f="compiler/tests/en/general/$name.salam"
    [ -e "$f" ] || continue
    gtotal=$((gtotal + 1))
    rout=$(timeout 15 "$REF" exec "$f" --no-color --log-level=error --lang=en 2>&1 | tr -d '\r')
    pout=$(timeout 15 "$PORT" "$f" en 2>&1 | tr -d '\r')
    if [ "$rout" = "$pout" ]; then
        gident=$((gident + 1))
    else
        {
            echo "=== MISMATCH: $f ==="
            echo "--- reference ---"
            echo "$rout"
            echo "--- port ---"
            echo "$pout"
            echo
        } >>"$GEN_MISMATCH_LOG"
    fi
done <"$CHUNK"

echo "$gident $gtotal" >>"$COUNT_FILE"
echo "chunk $CHUNK: $gident / $gtotal identical"
