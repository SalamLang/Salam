...
bool cli_parse_options(int argc, char **argv, int start, options_t *out)
{
    { int i = start; for (; i < argc; i++) {
        ...
        } else if (strcmp(arg, "-O0") == 0) { out->llvm_opt_level = 0;
        } else if (strcmp(arg, "-O1") == 0) { out->llvm_opt_level = 1;
        } else if (strcmp(arg, "-O2") == 0) { out->llvm_opt_level = 2;
        } else if (strcmp(arg, "-O3") == 0) { out->llvm_opt_level = 3;
        } else if (strcmp(arg, "-Os") == 0) { out->llvm_opt_level = 4;
        } else if (strcmp(arg, "-Oz") == 0) { out->llvm_opt_level = 5;
        } else if (strcmp(arg, "--emit-llvm") == 0) { out->llvm_emit = 0;
        } else if (strcmp(arg, "--emit-asm") == 0)  { out->llvm_emit = 1;
        } else if (strcmp(arg, "--emit-obj") == 0 || strcmp(arg, "-c") == 0) { out->llvm_emit = 2;
        } else if (strcmp(arg, "--emit-bitcode") == 0 || strcmp(arg, "-flto") == 0) { out->llvm_emit = 3;
        } else if (strcmp(arg, "--exec") == 0 || strcmp(arg, "--emit-exec") == 0) { out->llvm_emit = 4;
        } else if (strcmp(arg, "--jit") == 0 && out->command == CMD_LLVM) { out->llvm_emit = 5;
        } else if (strcmp(arg, "--verify-ir") == 0) { out->llvm_verify = true;
        } else if ((val = cli_opt_value(arg, "--target")) != NULL) { out->llvm_target = val;
