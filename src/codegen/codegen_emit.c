...
void cg_indent(cg_t *cg) { for (int i = 0; i < cg->indent; i++) sb_puts(cg->c, "    "); }
...
void cg_source_line(cg_t *cg, const src_span_t *span)
{
    ...
    for (const char *p = cg->src_path; *p && ei < sizeof(escaped) - 2; p++) {
        if (*p == '\\') escaped[ei++] = '\\';
        escaped[ei++] = *p;
    }
    escaped[ei] = '\0';
...
{
    for (size_t i = 0; i < cg->locals.len; i++)
        if (strcmp((const char *)cg->locals.data[i], name) == 0) return true;
    return false;
}

void local_add(cg_t *cg, const char *name) { vec_push(cg->a, &cg->locals, (void *)name); }
