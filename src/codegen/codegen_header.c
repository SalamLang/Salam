...
static void vec_collect_one(cg_t *cg, const char *ts)
{
    ...
    for (size_t i = 0; i < cg->vec_types.len; i++)
...
static void vec_collect(cg_t *cg, ast_node_t *n)
{
    ...
    for (size_t i = 0; i < n->list.len; i++) vec_collect(cg, (ast_node_t *)n->list.data[i]);
    for (size_t i = 0; i < n->dims.len; i++) vec_collect(cg, (ast_node_t *)n->dims.data[i]);
}
...
static void dyn_collect(cg_t *cg, ast_node_t *n)
{
    ...
    for (size_t i = 0; i < n->list.len; i++) dyn_collect(cg, (ast_node_t *)n->list.data[i]);
    for (size_t i = 0; i < n->dims.len; i++) dyn_collect(cg, (ast_node_t *)n->dims.data[i]);
}

static func_sig_t *dyn_match_sig(symbol_t *cm, func_sig_t *want)
{
    if (!cm) return NULL;
    for (size_t k = 0; k < cm->overloads.len; k++) {
...
{
    for (size_t i = 0; i < cg->dyn_ifaces.len; i++) {
        ...
        for (size_t m = 0; m < isym->members->symbols.len; m++) {
            ...
            for (size_t p = 0; p < sig->params.len; p++)
...
{
    for (size_t i = 0; i < cg->dyn_impls.len; i++) {
        ...
        for (size_t m = 0; m < isym->members->symbols.len; m++) {
            ...
            for (size_t p = 0; p < isig->params.len; p++) {
...
static void hdr_prelude(cg_t *cg, ast_node_t *program, sb_t *h)
{
    ...
    for (size_t i = 0; i < program->list.len; i++) {
        ...
        if (d->kind == AST_IMPORT) {
            ...
            for (const char *q = stem; *q && *q != '.' && k < sizeof(base)-1; q++) base[k++] = *q;
            base[k] = 0;
