...
static bool write_file(logger_t *log, const char *path, const char *content)
{
    ...
    if (!f) { LOG_E(log, PH_DRIVER, i18n_tr("cannot write '%s'"), path); return false; }
    fputs(content, f);
...
static void emit_link(sb_t *cmd, logger_t *log, const char *spec, const char *kind, bool use_tcc)
{
    bool framework = kind && strcmp(kind, "framework") == 0;
    bool is_static = kind && strcmp(kind, "static") == 0;
    if (framework) {
#if defined(__APPLE__)
        sb_puts(cmd, " -framework "); sb_puts(cmd, spec);
#else
        LOG_W(log, PH_DRIVER, "ignoring framework link '%s' (frameworks are macOS-only)", spec);
#endif
        return;
    }
...
int driver_build(options_t *opt)
{
    ...
    if (ndefs < SALAM_MAX_INPUTS)
        defs[ndefs++] = use_tcc ? "SALAM_CC_TCC"
    ...
    
    {
        ...
        if (mf) { fclose(mf); if (nwork < SALAM_MAX_INPUTS) work[nwork++] = memp; }
    }
    
    {
        const char *corep = salam_resolve_import(arena, "", "core");
        FILE *cf = corep ? fopen(corep, "rb") : NULL;
        if (cf) { fclose(cf); if (nwork < SALAM_MAX_INPUTS) work[nwork++] = corep; }
    }
    ...
    { int wi = 0; for (; wi < nwork; wi++) {
        ...
        { size_t k = 0; for (; k < program->list.len && nwork < SALAM_MAX_INPUTS; k++) {
            ...
            const char *spec = (d->value.kind == TV_STRING && d->value.as.s) ? d->value.as.s : d->name;
            if (!spec) continue;
    ...
    if (opt->command == CMD_OBJ) {
        ...
        if (opt->debug_info) {
            if (use_tcc)
                LOG_W(log, PH_DRIVER,
                      "tcc has limited debug-symbol support; prefer --cc=gcc or --cc=clang with -g");
            else
                sb_puts(&cmd, " -g");
        }
        if (opt->asan) {
