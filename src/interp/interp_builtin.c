...
{ (void)call; (void)n; return val_int(strcmp(to_str(I, a[0]), to_str(I, a[1]))); }

static value_t in_strtol(interp_t *I, ast_node_t *call, value_t *a, size_t n)
{ (void)call; (void)n; return val_int((int64_t)strtol(to_str(I, a[0]), NULL, 10)); }

static value_t in_strtod(interp_t *I, ast_node_t *call, value_t *a, size_t n)
{ (void)call; (void)n; return val_float(strtod(to_str(I, a[0]), NULL)); }

static value_t in_input(interp_t *I, ast_node_t *call, value_t *a, size_t n)
{ (void)call; (void)a; (void)n; return do_input(I); }

static value_t in_time(interp_t *I, ast_node_t *call, value_t *a, size_t n)
{ (void)I; (void)call; (void)a; (void)n; return val_int((int64_t)time(NULL)); }
#ifdef _WIN32
static value_t in_sleepex(interp_t *I, ast_node_t *call, value_t *a, size_t n)
{ (void)I; (void)call; (void)n; Sleep((DWORD)to_int(a[0])); return val_int(0); }
#else
static value_t in_usleep(interp_t *I, ast_node_t *call, value_t *a, size_t n)
{ (void)I; (void)call; (void)n; usleep((useconds_t)to_int(a[0])); return val_int(0); }
#endif
static const struct { const char *name; intrinsic_fn fn; } INTRINSICS[] = {
    {"sqrt", in_sqrt},   {"sin", in_sin},     {"cos", in_cos},     {"tan", in_tan},
    {"asin", in_asin},   {"acos", in_acos},   {"atan", in_atan},   {"atan2", in_atan2},
    {"log", in_log},     {"log10", in_log10}, {"exp", in_exp},     {"floor", in_floor},
    {"ceil", in_ceil},   {"fabs", in_fabs},   {"fmod", in_fmod},   {"pow", in_pow},
    {"strlen", in_strlen}, {"strcmp", in_strcmp}, {"strtol", in_strtol},
    {"strtod", in_strtod}, {"salam_input", in_input},
    {"time", in_time},
#ifdef _WIN32
    {"SleepEx", in_sleepex},
#else
                            ...
                            const char *method, value_t *args, size_t nargs)
{
    ...
    switch (recv.kind) {
        case VAL_STR: {
            ...
            if (!strcmp(method, "substr"))   return val_str(str_substr(I, s, to_int(a0), to_int(a1)));
            if (!strcmp(method, "find"))     { const char *h = strstr(s, to_str(I, a0));
        ...
        case VAL_ARRAY: {
            ...
            if (!strcmp(method, "push"))  { array_push(I, a, a0); return val_null(); }
            if (!strcmp(method, "pop"))   { if (a->len == 0) rt_error(I, call, "pop from empty vector");
                                            return a->data[--a->len]; }
            if (!strcmp(method, "get"))   { int64_t i = to_int(a0);
                                            if (i < 0 || (size_t)i >= a->len) rt_error(I, call, "vector index %lld out of range (len %zu)", (long long)i, a->len);
                                            
                                            sarray_t *box = array_new(I, 1); array_push(I, box, a->data[i]); return mk_array(I, box); }
            if (!strcmp(method, "set"))   { int64_t i = to_int(a0);
                                            if (i < 0 || (size_t)i >= a->len) rt_error(I, call, "vector index %lld out of range (len %zu)", (long long)i, a->len);
                                            a->data[i] = a1; return val_null(); }
        ...
        case VAL_MAP: {
            ...
            if (!strcmp(method, "put"))    { map_put(I, m, a0, a1); return val_null(); }
            if (!strcmp(method, "get"))    { smap_entry_t *e = map_find(m, a0); return e ? e->val : val_null(); }
            if (!strcmp(method, "has"))    return val_bool(map_find(m, a0) != NULL);
            if (!strcmp(method, "remove")) { smap_entry_t *e = map_find(m, a0);
                                             if (e) { e->used = false; m->count--; } return val_null(); }
            if (!strcmp(method, "size"))   return val_int((int64_t)m->count);
            if (!strcmp(method, "iter"))   { smapiter_t *it = (smapiter_t *)arena_alloc(I->a, sizeof *it);
                                             it->map = m; it->idx = 0;
                                             value_t v; v.kind = VAL_MAPITER; v.as.iter = it; return v; }
            break;
        }
        case VAL_MAPITER: {
            ...
            if (!strcmp(method, "next")) { while (it->idx < it->map->cap && !it->map->entries[it->idx].used) it->idx++;
                                           ...
                                           return val_null(); }
            if (!strcmp(method, "key")) { size_t i = it->idx; while (i < it->map->cap && !it->map->entries[i].used) i++;
                                          return i < it->map->cap ? it->map->entries[i].key : val_null(); }
            if (!strcmp(method, "value")) { size_t i = it->idx; while (i < it->map->cap && !it->map->entries[i].used) i++;
