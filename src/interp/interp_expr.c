...
static value_t eval_call(interp_t *I, env_t *env, ast_node_t *n)
{
    ...
    if (callee->kind == AST_IDENTIFIER) {
        ...
        if (!strcmp(nm, "print"))      { do_print(I, env, n, false, false); return val_null(); }
        if (!strcmp(nm, "println"))    { do_print(I, env, n, true,  false); return val_null(); }
        if (!strcmp(nm, "printerr"))   { do_print(I, env, n, false, true);  return val_null(); }
        if (!strcmp(nm, "printerrln")) { do_print(I, env, n, true,  true);  return val_null(); }
        ...
        size_t na; value_t *args = eval_args(I, env, n, &na);
        ...
        bool handled; value_t ir = call_intrinsic(I, n, nm, args, na, &handled);
...
value_t eval(interp_t *I, env_t *env, ast_node_t *n)
{
    ...
    switch (n->kind) {
        case AST_LITERAL:
            switch (n->op) {
                case TK_INT:          return val_int((int64_t)n->value.as.i);
                case TK_FLOAT:        return val_float(n->value.as.f);
                case TK_CHAR:         return val_char((int64_t)n->value.as.i);
                case TK_STRING:
                case TK_TRIPLE_STRING:
                case TK_RAW_STRING:
                case TK_UTF8_CHAR:    return val_str(n->value.as.s ? n->value.as.s : "");
                case TK_KW_TRUE:      return val_bool(true);
                case TK_KW_FALSE:     return val_bool(false);
                case TK_KW_NULL:      return val_null();
        ...
        case AST_BINARY: {
            ...
            if (op == TK_AND) { value_t a = eval(I, env, n->a);
                                return to_bool(a) ? val_bool(to_bool(eval(I, env, n->b))) : val_bool(false); }
            if (op == TK_OR)  { value_t a = eval(I, env, n->a);
                                return to_bool(a) ? val_bool(true) : val_bool(to_bool(eval(I, env, n->b))); }
            value_t a = eval(I, env, n->a);
            value_t b = eval(I, env, n->b);
            bool found; value_t r = try_struct_op(I, op, a, b, true, &found);
            if (found) return r;
            switch (op) {
                case TK_EQ: return val_bool(value_eq(I, a, b));
                case TK_NE: return val_bool(!value_eq(I, a, b));
                case TK_LT: return val_bool(value_cmp(I, a, b) <  0);
                case TK_GT: return val_bool(value_cmp(I, a, b) >  0);
                case TK_LE: return val_bool(value_cmp(I, a, b) <= 0);
                case TK_GE: return val_bool(value_cmp(I, a, b) >= 0);
                default:    return arith(I, n, op, a, b);
            }
        }
        case AST_UNARY: {
            value_t a = eval(I, env, n->a);
            if (a.kind == VAL_STRUCT) {
                bool found; value_t r = try_struct_op(I, n->op, a, val_null(), false, &found);
                if (found) return r;
            }
            
            if (n->op == TK_MINUS) return a.kind == VAL_FLOAT ? val_float(-a.as.f) : val_int((int64_t)(0 - (uint64_t)to_int(a)));
            if (n->op == TK_NOT)   return val_bool(!to_bool(a));
            return a;
        }
        case AST_CAST: {
            value_t a = eval(I, env, n->a);
            char base[96]; base_typename(n->type ? n->type->type_str : NULL, base, sizeof base);
        ...
        case AST_CALL: return eval_call(I, env, n);
        case AST_MEMBER: {
            ast_node_t *obj = n->a;
        ...
        case AST_INDEX: {
            ...
            if (a.kind == VAL_STRUCT) {
                ...
                if (m) { value_t idx = eval(I, env, n->b); return call_func(I, m, I->globals, &a, &idx, 1); }
            }
            value_t idx = eval(I, env, n->b);
            if (a.kind == VAL_ARRAY) {
                int64_t i = to_int(idx);
                if (i < 0 || (size_t)i >= a.as.arr->len)
                    rt_error(I, n, "index %lld out of range (len %zu)", (long long)i, a.as.arr->len);
                return a.as.arr->data[i];
            }
            if (a.kind == VAL_STR) {
                int64_t i = to_int(idx); size_t len = strlen(a.as.s);
