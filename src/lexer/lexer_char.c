...
{
    switch (base) {
        case 'x': case 'X': return lx_hex(c) >= 0;
...
int lx_decimal_digit(const char *s, size_t n, int *val)
{
    ...
    if (b0 >= '0' && b0 <= '9') { *val = b0 - '0'; return 1; }
    if (n < 2) return 0;
    unsigned char b1 = (unsigned char)s[1];
    if (b0 == 0xD9 && b1 >= 0xA0 && b1 <= 0xA9) { *val = b1 - 0xA0; return 2; }
    if (b0 == 0xDB && b1 >= 0xB0 && b1 <= 0xB9) { *val = b1 - 0xB0; return 2; }
    return 0;
}
