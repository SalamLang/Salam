...
char lx_peek(lx_t *L)            { return L->pos < L->n ? L->s[L->pos] : '\0'; }

char lx_peek2(lx_t *L, size_t k) { return L->pos + k < L->n ? L->s[L->pos + k] : '\0'; }

bool lx_end(lx_t *L)             { return L->pos >= L->n; }
...
{
    if (lx_peek(L) == c) { lx_adv(L); return true; }
    return false;
}

void lx_skip_ident(lx_t *L)
{
    while (lx_is_ident_cont(lx_peek(L))) lx_adv(L);
}
...
{
    size_t n = 0;
    while ((int)n < max && !lx_end(L) && lx_hex(lx_peek(L)) >= 0) { lx_adv(L); n++; }
    return n;
}

bool lx_expect(lx_t *L, char close, const src_pos_t *begin, const char *errmsg)
{
    if (lx_peek(L) == close) { lx_adv(L); return true; }
                 ...
                 const char *lexeme, const token_value_t *val)
{
    ...
    L->last = kind;
    L->has_last = true;
    LOG_T_AT(L->log, PH_LEXER, L->file, t.span,
             "token recognized: %s '%s'", token_kind_name(kind), lexeme ? lexeme : "");
}
