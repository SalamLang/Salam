...
void ll_emit_return(ll_t *ll, ast_node_t *value)
{
    ll_emit_defers(ll);
    if (ll->is_main) {
        if (value) ll_emit_term(ll, "ret i32 %s", ll_conv(ll, ll_expr(ll, value), "i32"));
        else       ll_emit_term(ll, "ret i32 0");
        return;
    }
    if (!ll->ret_ts || !strcmp(ll->ret_ts, "void")) { ll_emit_term(ll, "ret void"); return; }
...
{
    ast_node_t *tgt = n->a;
    if (!tgt || (tgt->kind != AST_IDENTIFIER && tgt->kind != AST_MEMBER && tgt->kind != AST_INDEX)) {
        ll_error(ll, n, "assignment to a non-lvalue");
        return;
    }
    
    if (n->op == TK_ASSIGN && tgt->kind == AST_INDEX && ll_index_set(ll, tgt, n->b)) return;
    ...
    if (n->op == TK_ASSIGN) {
        ...
        if (base == TK_EOF) { ll_error(ll, n, "compound assignment operator"); return; }
        
        ast_node_t bin = *n; bin.kind = AST_BINARY; bin.op = base;
        bin.a = tgt; bin.b = n->b; bin.type_str = ts;
...
static void ll_if(ll_t *ll, ast_node_t *n)
{
    ...
    if (n->c) {
        ...
        if (n->c->kind == AST_IF) ll_stmt(ll, n->c);   
        else                      ll_stmts_scoped(ll, n->c);
        ll_emit_term(ll, "br label %%%s", endL);
    }
...
static void ll_while(ll_t *ll, ast_node_t *n)
{
    ...
    ll_emit_term(ll, "br i1 %s, label %%%s, label %%%s", ll_as_i1(ll, ll_expr(ll, n->a)), bodyL, endL);
    ll_emit_label(ll, bodyL);
    if (ll->nloop >= 64) { ll_error(ll, n, "loop nesting too deep"); return; }  
    ll->brk[ll->nloop] = endL; ll->cont[ll->nloop] = condL; ll->nloop++;
    ll_stmts_scoped(ll, n->b);
    ll->nloop--;
    ll_emit_term(ll, "br label %%%s", condL);
    ll_emit_label(ll, endL);
...
static void ll_for(ll_t *ll, ast_node_t *n)
{
    ...
    ll_emit_term(ll, "br label %%%s", condL);
    ll_emit_label(ll, condL);
    if (n->b) ll_emit_term(ll, "br i1 %s, label %%%s, label %%%s", ll_as_i1(ll, ll_expr(ll, n->b)), bodyL, endL);
    else      ll_emit_term(ll, "br label %%%s", bodyL);
    ll_emit_label(ll, bodyL);
    if (ll->nloop >= 64) { ll_error(ll, n, "loop nesting too deep"); ll->locals.len = mark; return; }  
    ll->brk[ll->nloop] = endL; ll->cont[ll->nloop] = stepL; ll->nloop++;
...
void ll_stmt(ll_t *ll, ast_node_t *n)
{
    ...
    switch (n->kind) {
        case AST_BLOCK:      ll_block(ll, n); break;
        case AST_VAR_DECL:   ll_vardecl(ll, n); break;
        case AST_CONST_DECL: ll_vardecl(ll, n); break;
        case AST_ASSIGN:     ll_assign(ll, n); break;
        case AST_EXPR_STMT:  ll_expr(ll, n->a); break;
        case AST_RETURN:     ll_emit_return(ll, n->a); break;
        case AST_IF:         ll_if(ll, n); break;
        case AST_WHILE:      ll_while(ll, n); break;
        case AST_FOR:        ll_for(ll, n); break;
        case AST_REPEAT:     ll_repeat(ll, n); break;
        case AST_DEFER:      vec_push(ll->a, &ll->defers, n->a); break;
        case AST_BREAK:    if (ll->nloop) ll_emit_term(ll, "br label %%%s", ll->brk[ll->nloop - 1]); break;
        case AST_CONTINUE: if (ll->nloop) ll_emit_term(ll, "br label %%%s", ll->cont[ll->nloop - 1]); break;
        default:             ll_error(ll, n, "%s statement", ast_kind_name(n->kind)); break;
    }
}
