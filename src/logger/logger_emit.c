...
static void logger_emit_rich(logger_t *lg, log_level_t level,
                             ...
                             const char *fmt, va_list ap)
{
    ...
    if ((msg_buf[0] == 'E' || msg_buf[0] == 'W') &&
        (unsigned char)msg_buf[1] >= '0' && (unsigned char)msg_buf[1] <= '9' &&
        (unsigned char)msg_buf[2] >= '0' && (unsigned char)msg_buf[2] <= '9' &&
        (unsigned char)msg_buf[3] >= '0' && (unsigned char)msg_buf[3] <= '9' &&
        msg_buf[4] == ':' && msg_buf[5] == ' ')
    {
        is_error = (msg_buf[0] == 'E');
        code     = (msg_buf[1] - '0') * 100
                 + (msg_buf[2] - '0') * 10
                 + (msg_buf[3] - '0');
        body     = msg_buf + 6;
    }
    const char *diag_level = is_error ? "error" : "warning";
    diag_render(lg->sink, diag_level, code, is_error,
                file, span,
                lg->src_text, lg->src_len,
                body, NULL,
                lg->diag_style, lg->diag_format,
                lg->color);
}
...
{
    if (lg->timestamps)
        logger_write_timestamp(lg->sink);
    if (lg->color) {
        fprintf(lg->sink, "[%s][%s%s%s] ",
                i18n_tr(log_phase_name(phase)),
