...
ast_node_t *parse_top_level(parser_t *p)
{
    ...
    bool is_pub = false;
    if (p_at(p, TK_KW_PUB)) { is_pub = true; p_advance(p); }
    ast_node_t *n = NULL;
    switch (p_peek(p)->kind) {
        case TK_KW_TYPE:   n = parse_type_alias(p); break;
        case TK_KW_CONST:  n = parse_const(p); break;
        case TK_KW_ENUM:   n = parse_enum(p); break;
        case TK_KW_STRUCT: n = parse_struct(p); break;
        case TK_KW_INTERFACE: n = parse_interface(p); break;
        case TK_KW_IMPL:   n = parse_impl(p); break;
        case TK_KW_FUNC:   n = parse_function(p); break;
        case TK_KW_LAYOUT:
        ...
        case TK_KW_MUT:    n = parse_var_decl(p); p_term(p); break;
        case TK_IDENT:     n = parse_bare_var_decl(p); p_term(p); break;
        default:
...
ast_node_t *parse_const(parser_t *p)
{
    ...
    if (p_at(p, TK_COLON) || p_at(p, TK_IDENT) || p_at(p, TK_KW_FUNC)) n->type = parse_type_anno(p);
...
static ast_node_t *parse_field(parser_t *p)
{
    ...
    n->name = p_at(p, TK_IDENT) ? parse_decl_name(p)
                                : p_member_name(p, "expected field name");
...
static const char *parse_op_method_name(parser_t *p)
{
    ...
    switch (k) {
        case TK_PLUS:    p_advance(p); return "operator_add";
        case TK_MINUS:   p_advance(p); return "operator_sub"; 
        case TK_STAR:    p_advance(p); return "operator_mul";
        case TK_SLASH:   p_advance(p); return "operator_div";
        case TK_PERCENT: p_advance(p); return "operator_mod";
        case TK_POWER:   p_advance(p); return "operator_pow";
        case TK_EQ:      p_advance(p); return "operator_eq";
        case TK_NE:      p_advance(p); return "operator_ne";
        case TK_LT:      p_advance(p); return "operator_lt";
        case TK_GT:      p_advance(p); return "operator_gt";
        case TK_LE:      p_advance(p); return "operator_le";
        case TK_GE:      p_advance(p); return "operator_ge";
        case TK_NOT:     p_advance(p); return "operator_not"; 
        case TK_LBRACKET:
            p_advance(p);  
            p_expect(p, TK_RBRACKET, "']' in operator[]");
            if (p_at(p, TK_ASSIGN)) { p_advance(p); return "operator_index_set"; }
