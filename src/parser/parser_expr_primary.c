...
static bool lambda_ahead(const parser_t *p)
{
    ...
    size_t i = p->pos; int depth = 0;
    for (; i < p->count; i++) {
        token_kind_t k = token_stream_at(p->toks, i)->kind;
        if (k == TK_LPAREN) depth++;
        else if (k == TK_RPAREN) { if (--depth == 0) { i++; break; } }
        else if (k == TK_EOF) return false;
    }
...
static const char *builtin_callable_name(token_kind_t k)
{
    switch (k) {
        ...
        case TK_KW_PRINTLN:    return "println";
        case TK_KW_PRINTERR:   return "printerr";
        case TK_KW_PRINTERRLN: return "printerrln";
        case TK_KW_INPUT:      return "input";
        default:               return NULL;
    }
}
...
ast_node_t *parse_primary(parser_t *p)
{
    ...
    switch (t->kind) {
        case TK_INT: case TK_FLOAT: case TK_STRING: case TK_TRIPLE_STRING:
        case TK_RAW_STRING: case TK_CHAR: case TK_UTF8_CHAR:
        case TK_KW_TRUE: case TK_KW_FALSE: case TK_KW_NULL: {
            ast_node_t *n = p_mk(p, AST_LITERAL);
            n->op = t->kind; n->value = t->value; n->name = t->lexeme;
            p_advance(p); p_fin(p, n); return n;
        }
        case TK_KW_THIS: { ast_node_t *n = p_mk(p, AST_THIS); p_advance(p); p_fin(p, n); return n; }
        case TK_KW_PRINT: case TK_KW_PRINTLN:
        case TK_KW_PRINTERR: case TK_KW_PRINTERRLN: case TK_KW_INPUT: {
        ...
        case TK_LPAREN:
            ...
            { p_advance(p); ast_node_t *e = parse_expr(p); p_expect(p, TK_RPAREN, "')'"); return e; }
        case TK_LBRACKET: return parse_array_lit(p);
        case TK_IDENT: {
            if (p_peek2(p)->kind == TK_LBRACE && !p->no_struct_lit) return parse_struct_lit(p);
...
static ast_node_t *parse_struct_lit(parser_t *p)
{
    ...
    while (!p_at(p, TK_RBRACE) && !p_at_eof(p)) {
        ...
        fi->name = p_at(p, TK_IDENT) ? p_munch_name(p)
