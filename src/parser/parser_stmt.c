...
static ast_node_t *parse_statement(parser_t *p)
{
    ...
    switch (p_peek(p)->kind) {
        ...
        case TK_KW_PRINTLN:
        case TK_KW_PRINTERR:
        case TK_KW_PRINTERRLN: return parse_print_stmt(p);
        case TK_KW_MUT:    { ast_node_t *n = parse_var_decl(p); p_term(p); return n; }
        case TK_KW_CONST:  return parse_const(p);
        case TK_KW_IF:     return parse_if(p);
        case TK_KW_WHILE:  return parse_while(p);
        case TK_KW_FOR:    return parse_for(p);
        case TK_KW_REPEAT: return parse_repeat(p);
        case TK_KW_RET:    return parse_return(p);
        case TK_KW_DEFER:  return parse_defer(p);
        ...
        case TK_IDENT: {
            ...
            if (m == 1) {
                ...
                switch (after) {
                    case TK_ASSIGN: case TK_STMT_END: case TK_EOF:
                    case TK_COLON:  case TK_KW_FUNC:  case TK_AMP:
                    case TK_STAR:   case TK_LBRACKET: case TK_LT: case TK_DOT:
                        is_decl = true;  break;       
                    default:
                        is_decl = false; break;       
                }
...
static ast_node_t *parse_print_stmt(parser_t *p)
{
    ...
    if (p_at(p, TK_LPAREN)) {                        
        p_advance(p);
        if (!p_at(p, TK_RPAREN))
            do { ast_add(p->a, call, parse_expr(p)); } while (p_match(p, TK_COMMA));
        p_expect(p, TK_RPAREN, "')' to close arguments");
    } else if (!p_at(p, TK_STMT_END) && !p_at(p, TK_KW_END) &&
               !p_at(p, TK_RBRACE) && !p_at_eof(p)) {
...
static ast_node_t *parse_return(parser_t *p)
{
    ...
    p_advance(p);                                   
    if (!p_at(p, TK_STMT_END) && !p_at(p, TK_KW_END) && !p_at(p, TK_KW_ELSE) && !p_at_eof(p)) {
        n->a = parse_expr(p);
    }
    p_fin(p, n);
    p_term(p);
