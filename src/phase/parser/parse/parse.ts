    ...
    while (parser.index < parser.lexer.tokens.length) {
        ...
        if (token.type === TokenOtherType.TOKEN_EOF) {
            ...
            if (! function_declaration) {
                parser.pushError(parserMessageRenderer(parser.lexer.language.id, ParserMessageKeys.PARSER_FAILED_TO_PARSE_FUNCTION_STATEMENT));
                break;
            }
            if (! parser.ast.pushFunctionDeclaration(function_declaration)) {
                parser.pushError(parserMessageRenderer(parser.lexer.language.id, ParserMessageKeys.PARSER_FAILED_TO_PARSE_FUNCTION));
                break;
            }
        } else if (token.type === TokenKeywordType.TOKEN_LAYOUT) {
            const layout: AstLayout | undefined = parserParseLayout(parser);
            if (! layout) {
                parser.pushError(parserMessageRenderer(parser.lexer.language.id, ParserMessageKeys.PARSER_FAILED_TO_PARSE_LAYOUT_ELEMENT));
                break;
            }
            if (! parser.ast.setLayout(layout)) {
                parser.pushError(parserMessageRenderer(parser.lexer.language.id, ParserMessageKeys.PARSER_DUPLICATE_LAYOUT_DEFINITION));
                break;
            }
    } else {
            parser.pushError(parserMessageRenderer(parser.lexer.language.id, ParserMessageKeys.PARSER_UNEXPECTED_TOKEN_IN_PROGRAM, token.type));
            break;
        }
    }
};
