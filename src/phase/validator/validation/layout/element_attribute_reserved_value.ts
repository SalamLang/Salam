...
export function validateLayoutElementAttributeReservedValue(validator: Validator, attribute: AstLayoutAttribute, runtimeElementAttribute: RuntimeElementAttribute): string | undefined {
    const error: string = messageRenderer(validatorMessages, validator.ast.language.id, MessageKeys.VALIDATOR_ATTRIBUTE_RESERVED_VALUE_NOT_VALID, attribute.value.getString(), attribute.enduser_name, attribute.element_enduser_name);
