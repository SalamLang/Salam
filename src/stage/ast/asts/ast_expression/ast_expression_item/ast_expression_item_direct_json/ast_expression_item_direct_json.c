...
#define HANDLE_EXPR_JSON(TYPE, FIELD, FUNC, KEY)                          \
    ...
    switch (expression_item.type) {
        HANDLE_EXPR_JSON(AST_EXPRESSION_TYPE_LITERAL, literal,
                         ast_expression_literal_direct_json, "literal")
        HANDLE_EXPR_JSON(AST_EXPRESSION_TYPE_IDENTIFIER, identifier,
                         ast_expression_identifier_direct_json, "identifier")
        HANDLE_EXPR_JSON(AST_EXPRESSION_TYPE_BINARY, binary,
                         ast_expression_binary_direct_json, "binary")
        HANDLE_EXPR_JSON(AST_EXPRESSION_TYPE_UNARY, unary,
                         ast_expression_unary_direct_json, "unary")
        HANDLE_EXPR_JSON(AST_EXPRESSION_TYPE_INDEX, index,
                         ast_expression_index_direct_json, "index")
        HANDLE_EXPR_JSON(AST_EXPRESSION_TYPE_CALL, call,
                         ast_expression_call_direct_json, "call")

        default:
            log_info(
                "ast_expression_item_direct_json - Entering default case "
                "(unknown type)");
