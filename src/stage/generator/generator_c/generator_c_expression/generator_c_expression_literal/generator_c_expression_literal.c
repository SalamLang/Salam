...
buffer_t* generator_c_expression_literal(generator_t* generator, ast_t* ast) {
    ...
    switch (expression_literal.value->type) {
        ...
        case VALUE_TYPE_NUMBER_INT:
            buffer_append_str(
                temp,
                convert_int2string(expression_literal.value->raw.int_value));
            break;

        case VALUE_TYPE_NUMBER_FLOAT:
            buffer_append_str(temp,
                              convert_float2string(
                                  expression_literal.value->raw.float_value));
            break;

        case VALUE_TYPE_BOOL:
            buffer_append_str(temp, expression_literal.value->raw.bool_value
                                        ? "true"
                                        : "false");
            break;
        ...
        case VALUE_TYPE_SIZE:
            buffer_append_str(
                temp,
                convert_size2string(expression_literal.value->raw.size_value));
            break;
