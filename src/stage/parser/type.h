...
#define PARSER_CURRENT_NEXT \
    ((token_t*)parser->scanner->tokens->items[parser->index + 1])
#define PARSER_CURRENT_PREV \
    ((token_t*)parser->scanner->tokens->items[parser->index - 1])

#define PARSER_HAS_CURRENT                            \
    (parser->index < parser->scanner->tokens->size && \
     PARSER_CURRENT != NULL && PARSER_CURRENT->type != TOKEN_TYPE_EOF)
#define PARSER_HAS_NEXT                                   \
    (parser->index + 1 < parser->scanner->tokens->size && \
     PARSER_CURRENT_NEXT != NULL)
