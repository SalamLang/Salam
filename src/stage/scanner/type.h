...
#define SCANNER_HAS_NEXT \
    (scanner->index + 1 < scanner->length && SCANNER_CURRENT_NEXT != '\0')
...
#define SCANNER_CURRENT_LOCATION                                      \
    (token_location_t) {                                              \
        .begin_line = scanner->line, .begin_column = scanner->column, \
        .begin_index = scanner->index, .end_line = scanner->line,     \
        .end_column = scanner->column, .end_index = scanner->index,   \
        .length = 1,                                                  \
    }
