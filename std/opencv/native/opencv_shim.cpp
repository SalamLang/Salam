...
cv::Mat *as_mat(cv_mat_t *m) { return static_cast<cv::Mat *>(m); }

cv_mat_t *wrap(cv::Mat m) { return new cv::Mat(std::move(m)); }
...
KpBundle *as_kp(cv_keypoints_t *k) { return static_cast<KpBundle *>(k); }
...
ContoursBundle *as_contours(cv_contours_t *c) { return static_cast<ContoursBundle *>(c); }

cv::dnn::Net *as_net(cv_net_t *n) { return static_cast<cv::dnn::Net *>(n); }
...
int map_color_code(int32_t code, bool *ok) {
    *ok = true;
    switch (code) {
        case 0: return cv::COLOR_BGR2GRAY;
        case 1: return cv::COLOR_GRAY2BGR;
        case 2: return cv::COLOR_RGB2GRAY;
        case 3: return cv::COLOR_GRAY2RGB;
        case 4: return cv::COLOR_BGR2RGB;
        case 5: return cv::COLOR_RGB2BGR;
        case 6: return cv::COLOR_BGR2HSV;
        case 7: return cv::COLOR_HSV2BGR;
        case 8: return cv::COLOR_RGB2HSV;
        case 9: return cv::COLOR_HSV2RGB;
...
int map_interpolation(int32_t code) {
    switch (code) {
        ...
        default: return cv::INTER_LINEAR; // 1, and any unrecognized value
    }
}

int map_rotate90(int32_t code) {
    switch (code) {
        case 1: return cv::ROTATE_180;
        case 2: return cv::ROTATE_90_COUNTERCLOCKWISE;
        default: return cv::ROTATE_90_CLOCKWISE; // 0
    }
}

int map_flip(int32_t mode) {
    switch (mode) {
        case 1: return 1;   // FLIP_HORIZONTAL
        case 2: return -1;  // FLIP_BOTH
        default: return 0;  // FLIP_VERTICAL
    }
}
...
int map_thresh(int32_t kind, bool *otsu) {
    *otsu = (kind == 5);
    switch (kind) {
        ...
        default: return cv::THRESH_BINARY; // 0, and OTSU (5) rides on BINARY
    }
}
...
int map_morph_op(int32_t op) {
    switch (op) {
        case 1: return cv::MORPH_CLOSE;
        case 2: return cv::MORPH_GRADIENT;
        case 3: return cv::MORPH_TOPHAT;
        case 4: return cv::MORPH_BLACKHAT;
        default: return cv::MORPH_OPEN; // 0
    }
}

int map_norm(int32_t norm) { return norm == 1 ? cv::NORM_L2 : cv::NORM_HAMMING; }

} // namespace

// ---------------------------------------------------------------- diagnostics

const char *scv_last_error() { return g_last_error.c_str(); }
...
void scv_free(cv_mat_t *mat) { delete as_mat(mat); }
...
cv_mat_t *scv_new_blank(int32_t width, int32_t height, int32_t channels, int32_t r, int32_t g,
                         int32_t b) {
    clear_error();
...
cv_mat_t *scv_from_buffer(const uint8_t *data, int32_t width, int32_t height, int32_t channels) {
    clear_error();
...
int32_t scv_width(cv_mat_t *mat) { return mat ? as_mat(mat)->cols : 0; }
int32_t scv_height(cv_mat_t *mat) { return mat ? as_mat(mat)->rows : 0; }
int32_t scv_channels(cv_mat_t *mat) { return mat ? as_mat(mat)->channels() : 0; }
int32_t scv_empty(cv_mat_t *mat) { return (!mat || as_mat(mat)->empty()) ? 1 : 0; }
uint8_t *scv_data(cv_mat_t *mat) { return mat ? as_mat(mat)->data : nullptr; }
int32_t scv_step(cv_mat_t *mat) { return mat ? static_cast<int32_t>(as_mat(mat)->step[0]) : 0; }
int32_t scv_mat_total(cv_mat_t *mat) { return mat ? static_cast<int32_t>(as_mat(mat)->total()) : 0; }
...
int32_t scv_set_pixel(cv_mat_t *mat, int32_t x, int32_t y, int32_t r, int32_t g, int32_t b) {
    if (!mat) return 0;
    cv::Mat *m = as_mat(mat);
    if (x < 0 || y < 0 || x >= m->cols || y >= m->rows) return 0;
    if (m->channels() == 1) {
        ...
        m->at<cv::Vec3b>(y, x) = cv::Vec3b(static_cast<uint8_t>(b), static_cast<uint8_t>(g),
...
cv_mat_t *scv_resize(cv_mat_t *mat, int32_t width, int32_t height, int32_t interpolation) {
    ...
    try {
        cv::Mat out;
        cv::resize(*as_mat(mat), out, cv::Size(width, height), 0, 0, map_interpolation(interpolation));
        return wrap(std::move(out));
    } catch (const std::exception &e) {
        set_error(e.what());
        return nullptr;
...
cv_mat_t *scv_rotate_angle(cv_mat_t *mat, double angle_deg, double scale) {
    ...
    try {
        ...
        cv::Mat rot = cv::getRotationMatrix2D(center, angle_deg, scale <= 0.0 ? 1.0 : scale);
        cv::Mat out;
...
cv_mat_t *scv_adaptive_threshold(cv_mat_t *mat, double maxval, int32_t method, int32_t kind,
    ...
    if (bs < 3) bs = 3;
    try {
        int cv_method = method == 1 ? cv::ADAPTIVE_THRESH_GAUSSIAN_C : cv::ADAPTIVE_THRESH_MEAN_C;
...
int32_t scv_hist_gray(cv_mat_t *mat, int32_t *out_counts) {
    ...
    try {
        ...
        for (int y = 0; y < gray.rows; y++) {
            ...
            for (int x = 0; x < gray.cols; x++) out_counts[row[x]]++;
        }
        return 1;
    } catch (const std::exception &e) {
        set_error(e.what());
...
int32_t scv_rectangle(cv_mat_t *mat, int32_t x, int32_t y, int32_t w, int32_t h, int32_t r,
                       int32_t g, int32_t b, int32_t thickness) {
    if (!mat) return 0;
    cv::Mat *m = as_mat(mat);
    cv::rectangle(*m, cv::Rect(x, y, w, h), rgb_scalar(r, g, b, m->channels()), thickness);
    return 1;
}

int32_t scv_circle(cv_mat_t *mat, int32_t cx, int32_t cy, int32_t radius, int32_t r, int32_t g,
                    int32_t b, int32_t thickness) {
    if (!mat) return 0;
    cv::Mat *m = as_mat(mat);
    cv::circle(*m, cv::Point(cx, cy), radius, rgb_scalar(r, g, b, m->channels()), thickness);
    return 1;
}

int32_t scv_put_text(cv_mat_t *mat, const char *text, int32_t x, int32_t y, double font_scale,
...
std::vector<cv::Point> pts_from_xy(const int32_t *pts_xy, int32_t count) {
    ...
    pts.reserve(count);
    for (int32_t i = 0; i < count; i++) pts.emplace_back(pts_xy[i * 2], pts_xy[i * 2 + 1]);
    return pts;
}
} // namespace
...
int32_t scv_fill_poly(cv_mat_t *mat, const int32_t *pts_xy, int32_t count, int32_t r, int32_t g,
                       int32_t b) {
...
cv_mat_t *scv_erode(cv_mat_t *mat, int32_t ksize, int32_t iterations) {
    ...
    try {
        ...
        cv::Mat out;
        cv::erode(*as_mat(mat), out, kernel, cv::Point(-1, -1), iterations > 0 ? iterations : 1);
        return wrap(std::move(out));
    } catch (const std::exception &e) {
        set_error(e.what());
        return nullptr;
...
cv_mat_t *scv_dilate(cv_mat_t *mat, int32_t ksize, int32_t iterations) {
    ...
    try {
        cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(ksize, ksize));
        cv::Mat out;
        cv::dilate(*as_mat(mat), out, kernel, cv::Point(-1, -1), iterations > 0 ? iterations : 1);
        return wrap(std::move(out));
    } catch (const std::exception &e) {
        set_error(e.what());
        return nullptr;
...
cv_mat_t *scv_bitwise(cv_mat_t *mat1, cv_mat_t *mat2, int32_t op) {
    ...
    try {
        cv::Mat out;
        switch (op) {
            case 1: cv::bitwise_or(*as_mat(mat1), *as_mat(mat2), out); break;
            case 2: cv::bitwise_xor(*as_mat(mat1), *as_mat(mat2), out); break;
            default: cv::bitwise_and(*as_mat(mat1), *as_mat(mat2), out); break;
        }
        return wrap(std::move(out));
    } catch (const std::exception &e) {
        set_error(e.what());
...
cv_mat_t *scv_warp_perspective_quad(cv_mat_t *mat, const double *src_xy, const double *dst_xy,
    ...
    try {
        ...
        for (int i = 0; i < 4; i++) {
            src[i] = cv::Point2f(static_cast<float>(src_xy[i * 2]), static_cast<float>(src_xy[i * 2 + 1]));
            dst[i] = cv::Point2f(static_cast<float>(dst_xy[i * 2]), static_cast<float>(dst_xy[i * 2 + 1]));
        }
...
int32_t scv_contour_point_get(cv_contours_t *cs, int32_t idx, int32_t pt_idx, int32_t *out_xy) {
...
int32_t scv_draw_contour(cv_mat_t *mat, cv_contours_t *cs, int32_t idx, int32_t r, int32_t g,
                          int32_t b, int32_t thickness) {
    if (!mat || !cs) return 0;
    cv::Mat *m = as_mat(mat);
    cv::drawContours(*m, as_contours(cs)->contours, idx, rgb_scalar(r, g, b, m->channels()),
                      thickness);
    return 1;
}

void scv_free_contours(cv_contours_t *cs) { delete as_contours(cs); }

// -------------------------------------------------------------------- features

int32_t scv_good_features(cv_mat_t *mat, int32_t max_corners, double quality, double min_dist,
...
void scv_free_keypoints(cv_keypoints_t *kp) { delete as_kp(kp); }

cv_matches_t *scv_bf_match(cv_mat_t *desc1, cv_mat_t *desc2, int32_t max_matches, int32_t norm) {
    clear_error();
...
int32_t scv_match_count(cv_matches_t *m) { return m ? static_cast<int32_t>(as_matches(m)->size()) : 0; }

int32_t scv_match_get(cv_matches_t *m, int32_t idx, int32_t *out_query, int32_t *out_train,
                       double *out_dist) {
...
void scv_free_matches(cv_matches_t *m) { delete as_matches(m); }
...
void scv_cascade_free(cv_cascade_t *c) { delete static_cast<cv::CascadeClassifier *>(c); }
...
int32_t scv_cascade_detect(cv_cascade_t *c, cv_mat_t *mat, double scale_factor,
    ...
    try {
        ...
        static_cast<cv::CascadeClassifier *>(c)->detectMultiScale(
            *as_mat(mat), results, scale_factor > 1.0 ? scale_factor : 1.1, min_neighbors);
...
cv_net_t *scv_dnn_load(const char *model_path, const char *config_path) {
    ...
    try {
        std::string cfg = (config_path && std::strlen(config_path) > 0) ? config_path : std::string();
...
void scv_dnn_free(cv_net_t *net) { delete as_net(net); }

int32_t scv_dnn_set_input(cv_net_t *net, cv_mat_t *mat, double scale, double mean_r, double mean_g,
    ...
    try {
        ...
        cv::Mat blob = cv::dnn::blobFromImage(*as_mat(mat), scale, cv::Size(width, height),
                                               cv::Scalar(mean_b, mean_g, mean_r), swap_rb != 0,
                                               false, CV_32F);
...
int32_t scv_video_frame_count(cv_video_t *v) {
    return v ? static_cast<int32_t>(static_cast<cv::VideoCapture *>(v)->get(cv::CAP_PROP_FRAME_COUNT))
             : 0;
}

cv_writer_t *scv_writer_open(const char *path, const char *fourcc, double fps, int32_t width,
                              int32_t height) {
    clear_error();
