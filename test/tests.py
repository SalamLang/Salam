...
def run_tests_in_directory(directory):
	...
	if parent_layout_file.exists():
		...
		print(f"{COLOR_RED}{parent_layout_file} does not exist. Skipping salam command.{COLOR_RESET}")
...
def compare_output_to_expected(directory):
	...
	if comparison.diff_files:
		...
		warnings += 1
		print(f"{COLOR_BLUE}Warning: Extra files found in the output directory of {directory}:{COLOR_RESET}")
