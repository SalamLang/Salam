...
static int conn_open(struct worker *w, int ep, struct conn *c) {
    ...
    if (fd < 0) { w->st.connect_err++; return -1; }
    set_nonblock(fd);
    int one = 1;
...
static int conn_send(struct worker *w, int ep, struct conn *c) {
    ...
    if (n != (ssize_t)w->request_len) { w->st.io_err++; return -1; }
...
static void complete(struct worker *w, int ep, struct conn *c) {
    ...
    if (*w->phase == 2) { conn_close(w, ep, c); return; }
...
static void parse_headers(struct conn *c, const char *end_of_headers) {
    ...
    if (te && hdr_find(te, 40 < (size_t)(c->hdr + hl - te) ? 40 : (size_t)(c->hdr + hl - te), "chunked")) {
        c->mode = BODY_CHUNKED;
        c->chunk_need_size = 1;
        c->chunk_left = 0;
    } else if (cl) {
    ...
    if (c->mode == BODY_LENGTH) {
        ...
        if (spill > 0) { c->mode = BODY_UNTIL_CLOSE; c->will_close = 1; }
    }
}
...
static int chunked_consume(struct conn *c, const char *p, size_t n) {
    ...
    while (i < n) {
        if (c->chunk_need_size) {
            ...
            long long payload = (long long)take > c->chunk_left ? c->chunk_left : (long long)take;
...
static void on_readable(struct worker *w, int ep, struct conn *c) {
    ...
    for (;;) {
        ...
        if (n < 0) {
            ...
            w->st.io_err++;
            conn_close(w, ep, c);
            if (*w->phase != 2) { w->st.reconnects++; conn_open(w, ep, c); }
            return;
        }
        if (n == 0) {
        ...
        if (c->in_headers) {
            ...
            parse_headers(c, eoh);
            if (c->mode == BODY_LENGTH && c->remaining == 0) { complete(w, ep, c); return; }
            left -= take;
            p += take;
            if (left == 0) continue;
        }
        ...
        if (c->mode == BODY_LENGTH) {
            long long take = (long long)left < c->remaining ? (long long)left : c->remaining;
            c->remaining -= take;
            if (c->remaining == 0) { complete(w, ep, c); return; }
        } else if (c->mode == BODY_CHUNKED) {
            if (chunked_consume(c, p, left)) { complete(w, ep, c); return; }
        }
...
static void *run_worker(void *arg) {
    ...
    for (int i = 0; i < w->n_conns; i++) conn_close(w, ep, &w->conns[i]);
    close(ep);
    return NULL;
}
...
int main(int argc, char **argv) {
    struct config cfg = {
        .method = "GET", .connections = 64, .threads = 4,
        .duration_s = 10, .warmup_s = 2, .keepalive = 1, .json_out = 0,
    };
    const char *url = NULL;

    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-c") && i + 1 < argc) cfg.connections = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-t") && i + 1 < argc) cfg.threads = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-d") && i + 1 < argc) cfg.duration_s = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-w") && i + 1 < argc) cfg.warmup_s = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-m") && i + 1 < argc) snprintf(cfg.method, sizeof cfg.method, "%s", argv[++i]);
        else if (!strcmp(argv[i], "-b") && i + 1 < argc) { cfg.body = argv[++i]; cfg.body_len = strlen(cfg.body); }
        else if (!strcmp(argv[i], "-k") && i + 1 < argc) cfg.keepalive = atoi(argv[++i]);
        else if (!strcmp(argv[i], "-j")) cfg.json_out = 1;
        else url = argv[i];
    }
    if (!url || parse_url(&cfg, url) < 0) {
        fprintf(stderr, "usage: loadgen [-c N] [-t N] [-d SEC] [-w SEC] [-m METHOD] [-b BODY] [-k 0|1] [-j] URL\n");
        return 2;
    }
    ...
    if (rc != 0) { fprintf(stderr, "resolve %s:%s: %s\n", cfg.host, cfg.port, gai_strerror(rc)); return 2; }
    ...
    struct timespec ts;
    if (cfg.warmup_s > 0) { ts.tv_sec = cfg.warmup_s; ts.tv_nsec = 0; nanosleep(&ts, NULL); }
    long long t0 = now_ns();
    phase = 1;
    ts.tv_sec = cfg.duration_s; ts.tv_nsec = 0;
    nanosleep(&ts, NULL);
    long long t1 = now_ns();
    phase = 2;
    for (int i = 0; i < cfg.threads; i++) pthread_join(ws[i].th, NULL);
    ...
    for (int i = 0; i < cfg.threads; i++) {
        ...
        for (uint32_t b = 0; b < HIST_BUCKETS; b++) tot.hist[b] += ws[i].st.hist[b];
    }
    ...
    double sum_us = 0;
    for (uint32_t b = 0; b < HIST_BUCKETS; b++) sum_us += (double)b * tot.hist[b];
    double avg = tot.requests ? sum_us / (double)tot.requests : 0;

    if (cfg.json_out) {
        printf("{\"url\":\"%s\",\"connections\":%d,\"threads\":%d,\"duration_s\":%.3f,"
               ...
               url, cfg.connections, cfg.threads, secs,
               (unsigned long long)tot.requests, rps, rps * 60.0,
               secs > 0 ? (double)tot.bytes / secs : 0,
        ...
        printf("  requests   %llu  (%.0f/s, %.0f/min)\n",
               (unsigned long long)tot.requests, rps, rps * 60.0);
        printf("  latency    avg %.0fus  p50 %lluus  p90 %lluus  p99 %lluus  max %lluus\n",
               avg, (unsigned long long)p50, (unsigned long long)p90,
               (unsigned long long)p99, (unsigned long long)tot.max_us);
