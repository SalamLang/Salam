...
app.get('/json', (req, res) =>
    jsonOk(res, '{"message":"Hello, World!","server":"node","routes":13,"ok":true}'));

// Pays a full open/read/close per request, on purpose.
app.get('/file', (req, res) => {
    fs.readFile(path.join(ASSETS, 'data.json'), 'utf8', (err, body) => {
        if (err || !body) {
            res.status(500).type('application/json').send('{"error":"asset missing"}');
            return;
        }
        jsonOk(res, body);
    });
...
app.post('/echo', express.text({ type: '*/*', limit: '10mb' }), (req, res) => {
    ...
    jsonOk(res, `{"bytes":${Buffer.byteLength(body)},"echo":"${escapeJson(body)}"}`);
});
...
app.use('/static', express.static(ASSETS, { etag: false, lastModified: false }));
